...
/* ── Sub-components ─────────────────────────────────────────────────────────── */
const LeaveTypeCard = ({ type, selected, onClick }: { type: LeaveTypeOption, selected: boolean, onClick: () => void }) => {
...
const DayPreviewBanner = ({ days, calDays, contract, balance }: { days: number, calDays: number, contract: string, balance?: Balance }) => {
  if (!days) return null;
...
/* ── History Entry ──────────────────────────────────────────────────────────── */
const HistoryItem = ({ entry, onCancel, cancelling }: { entry: LeaveEntry, onCancel: ((id: string) => void) | null, cancelling: boolean }) => {
  ...
  return (
    <div className="p-4 hover:bg-slate-50 transition-colors duration-150">
      ...
      {!entry.approved && !entry.rejected && onCancel && (
        <button
          type="button"
          onClick={() => { const id = entry._id || entry.id; if (id) onCancel(id); }}
          disabled={cancelling}
