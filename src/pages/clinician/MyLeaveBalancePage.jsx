        ...
        {contracts.map((contract) => {
          ...
          return (
            <div key={contract} className="rounded-lg border border-slate-200 bg-white p-5">
              <Badge color={contract === "ARRS" ? "blue" : contract === "EA" ? "success" : "warning"}>{contract}</Badge>
              <div className="mt-5 text-2xl font-bold text-slate-900">{used} hours used / {total} hours total</div>
              <div className="mt-4 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-slate-700" style={{ width: `${pct}%` }} /></div>
            </div>
          );
        })}
      </div>
