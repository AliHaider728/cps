...
/* ── Component ──────────────────────────────────────────────────────── */
export default function CoverBookingModal({ open, onClose, gapShift, onAssign }) {
  const assignCover = useAssignCover();

  const [clinicianId,   setClinicianId]   = useState("");
  const [startTime,     setStartTime]     = useState(gapShift?.start_time ?? "09:00");
  const [endTime,       setEndTime]       = useState(gapShift?.end_time   ?? "17:00");
  ...
  const toggleCompliance = (id) => setCompliance((p) => ({ ...p, [id]: !p[id] }));

  /* ── Restriction check state ── */
  const [restrictionCheck,    setRestrictionCheck]    = useState(null); // null | { blocked, reason }
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-5">
          ...
          {assignCover.isError && (
            <div className="flex items-center gap-2 rounded-xl bg-red-50 border border-red-200 px-3 py-2.5 text-xs text-red-700">
              ...
              {String(assignCover.error?.message ?? "Failed to assign cover")}
            </div>
          )}
        </div>
