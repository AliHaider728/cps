...
// ✅ FIX: Aligned with backend SERVICE_CODES set (PCN, GP, EA only)
...
export default function AddShiftModal({ open, onClose, clinicianId, date }) {
  ...
  const handleStatus = (value) => {
    setStatus(value);
    if (value === "gap") { setSelectedClinicianId(""); setClinicianSearch(""); }
  };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Hourly Rate" icon={PoundSterling}>
              <input type="number" min="0" step="0.5" value={hourlyRate}
                onChange={(e) => setHourlyRate(e.target.value)} className={FIELD_CLASS} placeholder="0.00" />
            </Field>
