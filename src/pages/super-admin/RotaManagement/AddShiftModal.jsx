...
import {
  ...
  FileText, Loader2, Plus, PoundSterling, User, X, AlertCircle, CheckCircle2
} from "lucide-react";
...
function SuccessToast({ message, onDismiss }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] animate-in slide-in-from-bottom-4 fade-in duration-300">
      <div className="flex items-center gap-3 bg-white border border-emerald-200 shadow-lg shadow-emerald-100/60 rounded-2xl px-5 py-3.5 min-w-[280px]">
        <div className="h-8 w-8 rounded-xl bg-emerald-100 flex items-center justify-center shrink-0">
          <CheckCircle2 size={16} className="text-emerald-600" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-slate-800">Shifts created!</p>
          <p className="text-xs text-slate-500 truncate">{message}</p>
        </div>
        <button
          onClick={onDismiss}
          className="shrink-0 h-6 w-6 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 inline-flex items-center justify-center transition-colors"
        >
          <X size={12} />
        </button>
      </div>
...
export default function AddShiftModal({ open, onClose, clinicianId, date }) {
  ...
  const [rateFromMapping, setRateFromMapping] = useState(false); // flag: rate came from mapping
  ...
  /* ── Fetch project mappings when a clinician is selected ── */
  const { data: mappingData } = useQuery({
    queryKey: ["project-mappings", selectedClinicianId],
    queryFn:  () =>
      clinicianService.getProjectMappings(selectedClinicianId).then((r) => r.data),
  ...
  useEffect(() => {
    ...
    // Only set rate from clinician if no mapping rate is applied
    if (!rateFromMapping) {
      const rate = selectedClinician.hourly_rate ?? selectedClinician.hourlyRate ?? "";
      if (rate !== "") setHourlyRate(String(rate));
    }
  }, [selectedClinician]); // eslint-disable-line react-hooks/exhaustive-deps

  /* ── auto-fill rate from project mapping when practice is selected ── */
  useEffect(() => {
    if (!selectedPracticeId || projectMappings.length === 0) return;

    // Try to find a mapping that matches selected practice
    const match = projectMappings.find(
      (m) => String(m.practice_id) === String(selectedPracticeId)
    );

    if (match && match.rate != null) {
      setHourlyRate(String(Number(match.rate)));
      setRateFromMapping(true);
    }
  }, [selectedPracticeId, projectMappings]);
  ...
  const handleCreate = () => {
    ...
    if (daysOfWeek.length === 0)       { setValidationError("Please select at least one day of the week."); return; }
    if (status === "cover" && !serviceCode){ setValidationError("Cover shifts require a service code."); return; }
    ...
    create.mutate(payload, {
      onSuccess: (data) => {
        ...
        const msg      = [
          count ? `${count} shift${count !== 1 ? "s" : ""} added` : "Shifts added",
  ...
  /* ── find mapping hint for selected practice ── */
  ...
  return (
    <>
      {toastMsg && (
        <SuccessToast message={toastMsg} onDismiss={() => setToastMsg("")} />
      )}
            ...
            <div className="px-5 py-5 space-y-5">
              ...
              <SearchSelect
                ...
                onSelect={(c) => {
                  ...
                  setRateFromMapping(false); // reset so mapping re-evaluates
                }}
              ...
              {mappingHint && (
                <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-indigo-50 border border-indigo-200 text-xs text-indigo-700 font-semibold">
                  ...
                  Project mapping found: <span className="font-bold">{mappingHint.project}</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="Date To" icon={CalendarDays} required>
                  <input type="date" value={dateTo} min={dateFrom}
                    onChange={(e) => setDateTo(e.target.value)} className={FIELD_CLASS} />
                </Field>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Hourly Rate" icon={PoundSterling}>
                  <div className="relative">
                    ...
                    />
                    {rateFromMapping && (
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-indigo-500 pointer-events-none">
                        mapping
                      </span>
                    )}
                  </div>
