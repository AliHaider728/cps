...
const serviceCodeFromContract = (contract: string) => {
...
══════════════════════════════════════════════ */
export default function AddShiftModal({ open, onClose, clinicianId, date, editShift = null }: AddShiftModalProps) {
  ...
  const practices = useMemo(() => {
    const list = (practicesRes as { practices?: unknown, data?: unknown })?.practices ?? (practicesRes as { practices?: unknown, data?: unknown })?.data ?? practicesRes ?? [];
    return Array.isArray(list) ? list : [];
  }, [practicesRes]);
  ...
  const { data: mappingData } = useQuery({
    ...
    queryFn:  () => clinicianService.getProjectMappings(selectedClinicianId).then((r: any) => r.data),
    enabled:  !!selectedClinicianId,
    staleTime: 60_000,
  });
  const projectMappings: any[] = mappingData?.mappings ?? [];
  const mappingHint = selectedPracticeId && selectedClinicianId
    ? projectMappings.find((m: any) => String(m.practice_id) === String(selectedPracticeId))
    : null;
  ...
  useEffect(() => {
    ...
    setValidationError("");
    if (create.reset) create.reset();
    if (update.reset) update.reset();
  ...
  const toggleDay = (day: number) => setDaysOfWeek(cur => cur.includes(day) ? cur.filter(d => d !== day) : [...cur, day]);
  const handleStatus = (v: string) => { setStatus(v); if (v === "gap") { setSelectedClinicianId(""); setClinicianSearch(""); } };
          ...
          <div className="relative w-full max-w-2xl rounded-2xl bg-white shadow-2xl ring-1 ring-slate-900/5 max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-5 py-5 space-y-5">
              ...
              <SearchSelect
                ...
                onChange={(v: string) => { setClinicianSearch(v); setClinicianOpen(true); }}
                ...
                renderItem={(c: any) => c.fullName || c.name || "Unknown"}
                subItem={(c: any) => c.email}
                onSelect={(c: any) => { setSelectedClinicianId(String(c._id || c.id || "")); setClinicianSearch(c.fullName || c.name || ""); setClinicianOpen(false); setRateFromMapping(false); }}
              ...
              <SearchSelect
                ...
                onChange={(v: string) => { setPracticeSearch(v); setPracticeOpen(true); }}
                ...
                renderItem={(p: any) => p.name || "Unknown"}
                subItem={(p: any) => p.odsCode || p.ods_code}
                onSelect={(p: any) => {
...
function SearchSelect({ label, icon, required, value, onChange, onFocus, onBlur, placeholder, disabled, open, items, renderItem, subItem, onSelect, emptyAction, emptyLabel }: any) {
  const Icon = icon;
  return (
