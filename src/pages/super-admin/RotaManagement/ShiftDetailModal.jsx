...
export default function ShiftDetailModal({ open, onClose, shift, readOnly = false }) {
  ...
  const [status,        setStatus]        = useState(String(shift?.status || "working"));
  const [notes,         setNotes]         = useState(String(shift?.workstreams_notes || ""));
          ...
          {confirmDelete && (
            <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3">
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => { del.mutate(shift.id); onClose?.(); }}
        ...
        <div className="px-5 pb-5 pt-4 border-t border-slate-100 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {!effectiveReadOnly && (
              <button
                type="button"
                onClick={() => update.mutate({ id: shift.id, data: { status, workstreams_notes: notes } })}
