...
const getStatus = (s: string): StatusConfigItem => STATUS_CONFIG[s] ?? { bg: "bg-slate-100", text: "text-slate-500", border: "border-slate-200", dot: "bg-slate-300", label: s, short: "??", Icon: AlertTriangle };
...
export default function WeeklyView({
  ...
  const [addClinicianId, setAddClinicianId] = useState<string | undefined>(undefined);
  ...
  const clinicians: ClinicianRow[] = data?.data?.clinicians ?? data?.clinicians ?? [];
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="p-6 text-sm text-red-600">{String((error as Error)?.message ?? "Failed to load")}</div>
        ) : clinicians.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      {days.map((d, idx) => {
                        ...
                        const { Icon }  = cfg ?? ({} as StatusConfigItem);
