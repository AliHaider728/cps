...
import CoverBookingModal, { GapShift } from "./CoverBookingModal";
import {
  Briefcase,
  Umbrella,
  Thermometer,
...
/* ── Helpers ─────────────────────────────────────────────────────────── */
const monthName = (m: number, fmt: "long" | "short" | "narrow" = "long") =>
...
const getStatus = (s: string): StatusConfigItem =>
...
interface CalendarGridProps {
  ...
  year: number;
  clinicians: any[];
  onEventClick: (shift: any) => void;
  onGapClick: (shift: any) => void;
}

function CalendarGrid({ month, year, clinicians, onEventClick, onGapClick }: CalendarGridProps) {
  ...
  const getEventsForDay = (date: Date): ShiftEvent[] => {
...
function Sidebar({ month, year, clinicians, onNavigate }: SidebarProps) {
  const today    = new Date();
  const isoFn    = (d: Date) => new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate())).toISOString().slice(0, 10);
  ...
  const Section = ({ label, accentColor, list }: { label: string; accentColor: string; list: any[] }) => (
...
export default function MonthlyCalendarView({
  ...
  const [gapShift,    setGapShift]    = useState<GapShift | null>(null);
