...
export default function TimesheetQueuePage() {
  ...
  const stats = useMemo(() => ({
    ...
    approved: (history?.items || []).filter((i) => i.status === "approved").length,
    rejected: (history?.items || []).filter((i) => i.status === "rejected").length,
  }), [pending, history]);
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {rows.map((sheet) => (
              <tr key={sheet.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-4 py-3">
                  {/* ✅ FIX: path was /super-admin/rota/timesheets/:id — corrected */}
                  <Link
