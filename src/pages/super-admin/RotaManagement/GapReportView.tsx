...
export default function GapReportView() {
  ...
  const gaps: any[] = data?.data?.gaps || data?.gaps || [];
  const coverRequestsData: any[] = coverRequests?.data || [];
    ...
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-4">
        {isLoading ? (
          ...
          <div className="p-6 text-sm text-red-600">{String((error as any)?.message || "Failed")}</div>
        ) : gaps.length === 0 ? (
