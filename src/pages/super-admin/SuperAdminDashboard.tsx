...
function RAGBadge({ status }: { status: "green" | "amber" | "red" }) {
  ...
  const labels: Record<string, string> = { green: "On Track", amber: "Due Soon", red: "Overdue" };
  return (
...
function TsBadge({ status }: { status?: string }) {
  const map: Record<string, string> = {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider ${map[s] || map.draft}`}>
      {status || "draft"}
    </span>
  );
}
...
function StatCard({ icon: Icon, label, value, sub, gradient, ring, accent, glow, alert, onClick }: StatCardProps) {
  return (
    <button
      onClick={onClick}
...
function SectionHeader({ icon: Icon, title, count, countColor = "bg-slate-100 text-slate-600", onViewAll }: SectionHeaderProps) {
  return (
...
/* ─── Empty State ───────────────────────────────────────────────────────── */
function EmptyState({ icon: Icon = CheckCircle2, message }: { icon?: any, message: string }) {
  return (
...
export default function SuperAdminDashboard() {
  ...
  const allPCNs: PCN[] = useMemo(() => {
    ...
    const typedPcnData = pcnData as { pcns?: PCN[], data?: PCN[] };
    if (Array.isArray(typedPcnData.pcns))   return typedPcnData.pcns as unknown as PCN[];
    if (Array.isArray(typedPcnData.data))   return typedPcnData.data as unknown as PCN[];
    return [];
  }, [pcnData]);
  ...
  const allPractices: Practice[] = useMemo(() => {
    ...
    const typedPracticeData = practiceData as { practices?: Practice[], data?: Practice[] };
    if (Array.isArray(typedPracticeData.practices))   return typedPracticeData.practices as unknown as Practice[];
    if (Array.isArray(typedPracticeData.data))        return typedPracticeData.data as unknown as Practice[];
    return [];
  }, [practiceData]);

  const expiringDocs: ExpiringDoc[] = useMemo(() => {
    if (!expiryData) return [];
    if (Array.isArray(expiryData))              return expiryData;
    const typedExpiryData = expiryData as { documents?: ExpiringDoc[], data?: ExpiringDoc[] };
    if (Array.isArray(typedExpiryData.documents))    return typedExpiryData.documents;
    if (Array.isArray(typedExpiryData.data))         return typedExpiryData.data;
    return [];
  }, [expiryData]);
