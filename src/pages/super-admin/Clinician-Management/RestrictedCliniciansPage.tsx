/**
 * pages/super-admin/Clinician-Management/RestrictedCliniciansPage.tsx
 *
...
const EntityBadge = ({ type }: { type: string }) => {
...
/* ─── hooks  ── */
const useGlobalRestricted = (params: any) =>
  useQuery({
    queryKey: [QK.CLINICIANS, "restricted-global", params],
    queryFn:  () => clinicianService.getAll({ ...params, restricted: "true" }).then((r: any) => r.data),
  });

const usePerClientRestricted = (params: any) =>
  useQuery({
    queryKey: [QK.RESTRICTED_CLINICIANS, params],
    queryFn:  () => apiClient.get("/restricted-clinicians", { params }).then((r: any) => r.data),
  });
...
const RemoveModal = ({ record, clinicianName, onClose, onConfirm, loading }: { record: any, clinicianName: string, onClose: () => void, onConfirm: (r: any, reason: string) => void, loading: boolean }) => {
...
export default function RestrictedCliniciansPage() {
  ...
  const [removeModal, setRemoveModal] = useState<{ record: any, clinicianName: string } | null>(null);
  ...
  const entitiesAffected = new Set(clientItems.map((r: any) => r.entityId)).size;

  /* ── handlers  ─── */
  const handleRemovePerClient = async (record: any, reason: string) => {
    setRemoving(true);
    try {
  ...
  const globalColumns = useMemo(() => ([
    ...
    {
      header: "GPhC",
      cell: (c: any) => <span className="text-xs font-mono text-slate-600">{c.gphcNumber || "—"}</span>,
    },
    {
    ...
    {
      header: "Ops Lead",
      cell: (c: any) => <span className="text-xs text-slate-600">{c.opsLeadName || "—"}</span>,
    },
    {
      header: "Actions",
      cell: (c: any) => (
  ...
  const clientColumns = useMemo(() => ([
    ...
    {
      header: "Client type",
      cell: (r: any) => <EntityBadge type={r.entityType} />,
    },
    {
      header: "Client ID",
      cell: (r: any) => <span className="text-xs font-mono text-slate-500">{r.entityId || "—"}</span>,
    },
    {
      header: "Reason",
      cell: (r: any) => (
