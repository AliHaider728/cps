...
interface CPPEPanelProps {
  clinicianId: string;
  canManage?: boolean;
}

export default function CPPEPanel({ clinicianId, canManage }: CPPEPanelProps) {
  const { data, isLoading } = useClinicianCPPE(clinicianId);
  const saveM = useUpdateClinicianCPPE(clinicianId);

  const [state, setState] = useState<CPPEState>({
  ...
  const set = (patch: Partial<CPPEState>) => { setState((s) => ({ ...s, ...patch })); setDirty(true); };

  const updateMod = (idx: number, patch: Partial<CPPEModule>) => {
    setState((s) => {
