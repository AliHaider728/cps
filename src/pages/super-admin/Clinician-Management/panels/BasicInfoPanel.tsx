...
import { Btn, FormField, DetailRow, Spinner } from "./shared";
...
export default function BasicInfoPanel({
  ...
  const [extraSpec, setExtraSpec] = useState<string>("");
  const [editingSpec, setEditingSpec] = useState<number | null>(null);  // index being edited
  const [editSpecVal, setEditSpecVal] = useState<string>("");
  const [future, setFuture] = useState<string>("");
  const [skillsDirty, setSkillsDirty] = useState<boolean>(false);
  const [skillsSaving, setSkillsSaving] = useState<boolean>(false);
  const [specToDelete, setSpecToDelete] = useState<string | null>(null);  // spec string pending delete confirm
  ...
  const startEditSpec = (idx: number) => {
    setEditingSpec(idx);
  ...
  /* ── User label helpers ─────────────────────── */
  const userLabel = (u: any) => u?.fullName || u?.name || u?.email || "User";

  const userOpts: [string, string][] = (users || []).map((u: any) => [
    u._id || u.id,
    `${userLabel(u)} (${u.role || "user"})`,
  ]);

  const opsLeadName = (() => {
    const u = users.find((x: any) => String(x._id || x.id) === String(form.opsLead || clinician?.opsLead?._id || clinician?.opsLead));
    return userLabel(u) || userLabel(clinician?.opsLead) || "";
  })();
  const supervisorName = (() => {
    const u = users.find((x: any) => String(x._id || x.id) === String(form.supervisor || clinician?.supervisor?._id || clinician?.supervisor));
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        ...
        {!editing ? (
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <FormField label="Working hours"  value={form.workingHours}  onChange={(v: any) => set("workingHours")(Number(v) || 0)} type="number" />
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        ...
        <FormField
          ...
          value={future}
          onChange={(v: string) => { setFuture(v); setSkillsDirty(true); }}
          textarea
          rows={5}
