...
export default function CompliancePanel({ clinicianId, canManage }) {
  ...
  const totalUploaded = groupsData?.totalUploaded ?? legacyDocs.filter(d => d.status === "approved").length;
  const totalMissing  = groupsData?.totalMissing  ?? legacyDocs.filter(d => d.status === "missing").length;
  ...
  const isGroupExpanded = (id) => expandedGroups[id] !== false; // default open

  /* ── Assign modal ── */
  const openAssignModal = () => {
    setSelectedGroups(groups.map(g => String(g._id)));
    setAssignError("");
    setAssignModal(true);
  };
        ...
        >
          ...
          {upsertM.isError && (
            <div className="p-3 rounded-xl bg-red-50 border border-red-200 flex items-center gap-2 text-sm text-red-700">
              ...
              {upsertM.error?.response?.data?.message || "Upload failed. Please try again."}
            </div>
          )}
        </ModalShell>
      )}
