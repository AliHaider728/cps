...
export default function ClientHistoryPanel({ clinicianId, canManage, pcns = [], practices = [] }: ClientHistoryPanelProps) {
  const qc = useQueryClient();

  const { data, isLoading } = useQuery({
    queryKey: QK.CLINICIAN_CLIENT_HISTORY(clinicianId),
    queryFn:  () => clinicianService.getClientHistory(clinicianId).then((r: any) => r.data),
    enabled:  !!clinicianId,
  });
  ...
  const [modal, setModal] = useState<"add" | { end: true } | null>(null);
  const [form,  setForm]  = useState<ClientHistoryForm>({});
  ...
  const practiceOpts: [string, string][] = practices.map((p: any) => [p._id, p.practiceName || p.name]);
      ...
      {modal === "add" && (
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormField label="Start date"    value={form.startDate || ""}    onChange={(v: string) => setForm((f) => ({ ...f, startDate: v }))} type="date" required />
            <FormField label="Hours / week"  value={form.hoursPerWeek ?? ""} onChange={(v: string) => setForm((f) => ({ ...f, hoursPerWeek: Number(v) || 0 }))} type="number" />
          </div>
          <FormField label="Notes" value={form.notes || ""} onChange={(v: string) => setForm((f) => ({ ...f, notes: v }))} textarea rows={3} />
        </ModalShell>
      )}

      {modal && typeof modal === 'object' && modal.end && (
        <ModalShell
        ...
        >
          <FormField label="End date" value={form.endDate || ""} onChange={(v: string) => setForm((f) => ({ ...f, endDate: v }))} type="date" required />
          <FormField label="Reason"   value={form.reason || ""}  onChange={(v: string) => setForm((f) => ({ ...f, reason: v }))} textarea rows={3} />
        </ModalShell>
      )}
    </div>
  );
