...
export default function ScopePanel({ clinician, canRestrict, canManage }: ScopePanelProps) {
  ...
  const restrictM = useMutation({
    mutationFn: (reason: string) => clinicianService.restrict(id, reason).then((r: any) => r.data),
    onSuccess: () => {
  ...
  const unrestrictM = useMutation({
    mutationFn: () => clinicianService.unrestrict(id).then((r: any) => r.data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: QK.CLINICIAN(id) });
      qc.invalidateQueries({ queryKey: QK.CLINICIANS });
    },
  ...
  const scopeM = useMutation({
    mutationFn: (patch: any) => clinicianService.update(id, patch).then((r: any) => r.data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: QK.CLINICIAN(id) });
    },
  });
  ...
  // ─── Local state ────────────────────────────────────────────
  const [restrictModal, setRestrictModal] = useState<boolean>(false);
  const [reason,        setReason]        = useState<string>("");
  const [editing,       setEditing]       = useState<boolean>(false);
  const [saving,        setSaving]        = useState<boolean>(false);

  const [workstreams,        setWorkstreams]        = useState<string[]>([]);
  const [systemsInUse,       setSystemsInUse]       = useState<string[]>([]);
  const [shadowingAvailable, setShadowingAvailable] = useState<boolean>(false);
  const [extraWS,            setExtraWS]            = useState<string>("");
  const [extraSys,           setExtraSys]           = useState<string>("");
          ...
          <div className="flex items-center gap-2.5">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${restricted ? "bg-red-50" : "bg-green-50"}`}>
              {restricted
                ...
                : <ShieldCheck size={18} className="textgreen-600" />}
            </div>
