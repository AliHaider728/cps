 ...
 * Updated: better UI, larger fonts, professional colors, no sidebar
 */
...
export default function HierarchyTree({
  ...
  if (!tree?.length) return (
    <div className="flex flex-col items-center justify-center py-20 gap-3 px-6">
      ...
      <p className="text-sm text-slate-500 font-semibold">No clients found</p>
...
function ICBCard({ icb, expanded, toggle, selected, onSelect, isAdmin,
  ...
  return (
    <div className="rounded-2xl border border-blue-100 bg-white shadow-sm overflow-hidden">
      ...
      <div
        onClick={() => toggle(key)}
        className="flex items-center gap-3 px-4 py-3.5 cursor-pointer hover:bg-blue-50/60 transition-colors select-none">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-slate-800 truncate leading-tight">{icb.name}</p>
        ...
        {isAdmin && (
          <div className="flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
            ...
            <ActionBtn icon={Trash2} onClick={() => onDeleteICB(icb._id)} hover="hover:text-red-500 hover:bg-red-50" />
          </div>
        )}
      ...
      {/* Expanded body */}
      {isOpen && (
        <div className="border-t border-blue-50 bg-slate-50/50 p-3 space-y-2">
          ...
          {isAdmin && (
            <div className="flex gap-2 pt-1.5 flex-wrap">
              {onAddFederation && (
                <DashedBtn color="indigo" label="Add Federation" onClick={() => onAddFederation(icb._id)} />
              )}
              {onAddPCN && (
                <DashedBtn color="purple" label="Add PCN" onClick={() => onAddPCN({ icb: icb._id })} />
              )}
            </div>
          )}
        </div>
...
function FederationCard({ fed, expanded, toggle, selected, onSelect, isAdmin,
  ...
  return (
    <div className="rounded-xl border border-indigo-100 bg-white overflow-hidden">
      ...
      <div
        onClick={() => toggle(key)}
        className="flex items-center gap-3 px-3.5 py-3 cursor-pointer hover:bg-indigo-50/50 transition-colors select-none">
        <div className="w-8 h-8 rounded-lg bg-indigo-100 flex items-center justify-center shrink-0">
          <Layers size={14} className="text-indigo-600" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-slate-700 truncate">{fed.name}</p>
        ...
        {isAdmin && (
          <div className="flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
            <ActionBtn icon={Pencil} onClick={() => onEditFederation(fed)} hover="hover:text-blue-600 hover:bg-blue-50" />
            <ActionBtn icon={Trash2} onClick={() => onDeleteFederation(fed._id)} hover="hover:text-red-500 hover:bg-red-50" />
          </div>
        )}
      ...
      {isOpen && (
        <div className="border-t border-indigo-50 bg-indigo-50/20 px-3 py-2.5 space-y-2">
          {pcns.map(pcn => (
            <PCNCard
              key={pcn._id}
              pcn={pcn}
          ...
          {pcns.length === 0 && (
            <p className="text-xs text-slate-400 italic text-center py-2">No PCNs</p>
          )}
          {isAdmin && onAddPCN && (
            <DashedBtn color="purple" label="Add PCN" onClick={() => onAddPCN({ federation: fed._id })} />
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className={`rounded-xl border overflow-hidden transition-all
      ...
      <div
        ...
        {isAdmin && (
          <div className="flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
            <ActionBtn icon={Pencil} onClick={() => onEditPCN(pcn)} hover="hover:text-blue-600 hover:bg-blue-50" />
            <ActionBtn icon={Trash2} onClick={() => onDeletePCN(pcn._id)} hover="hover:text-red-500 hover:bg-red-50" />
          </div>
        )}
        <button
      ...
      {isOpen && (
        <div className="border-t border-slate-100 bg-slate-50/40 px-3 py-2 space-y-1">
          {practices.map(pr => (
            <PracticeRow
              key={pr._id}
              pr={pr}
          ...
          {practices.length === 0 && (
            <p className="text-xs text-slate-400 italic text-center py-2">No practices</p>
          )}
          {isAdmin && onAddPractice && (
            <DashedBtn color="emerald" label="Add Practice" onClick={() => onAddPractice({ pcn: pcn._id })} />
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div
      ...
      <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0
        ...
        <Stethoscope size={12} className={isActive ? "text-emerald-700" : "text-emerald-500"} />
      </div>
      ...
      {pr.odsCode && (
        <span className="text-xs font-mono text-slate-400 hidden group-hover:block shrink-0 bg-slate-100 px-1.5 py-0.5 rounded">
          {pr.odsCode}
        </span>
      )}
      {isAdmin && (
        <div className="hidden group-hover:flex items-center gap-0.5 shrink-0"
          ...
          <ActionBtn icon={Pencil} onClick={() => onEdit(pr, pcnId)} hover="hover:text-blue-600 hover:bg-blue-50" />
          <ActionBtn icon={Trash2} onClick={() => onDelete(pr._id)} hover="hover:text-red-500 hover:bg-red-50" />
        </div>
      )}
    </div>
  );
...
/* ── Primitives ─────────────────────────────────────────────────────────── */
const DASHED_COLORS = {
  indigo:  "border-indigo-200 text-indigo-500 hover:bg-indigo-50 hover:border-indigo-300",
  purple:  "border-purple-200 text-purple-500 hover:bg-purple-50 hover:border-purple-300",
  ...
  return (
    <button onClick={onClick}
      className={`w-full flex items-center justify-center gap-1.5 py-2 rounded-xl
        border border-dashed text-xs font-semibold transition-all
        ${DASHED_COLORS[color]}`}>
      <Plus size={12} /> {label}
    </button>
  );
...
function ActionBtn({ icon: Icon, onClick, hover, size = 13 }) {
  return (
    <button onClick={onClick}
