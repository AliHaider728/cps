...
import {
  ...
  FileSignature, AlertTriangle, Monitor, StickyNote, FileText,
} from "lucide-react";

import { hierarchyAPI, icbAPI, federationAPI, pcnAPI, practiceAPI, historyAPI } from "../../../api/api.js";
...
function HierarchyView() {
  ...
  const handleSave = async (type) => {
    ...
    try {
      ...
      const apiMap = {
        icb:        { create: icbAPI.create,        update: (id, d) => icbAPI.update(id, d) },
        federation: { create: federationAPI.create, update: (id, d) => federationAPI.update(id, d) },
        pcn:        { create: pcnAPI.create,        update: (id, d) => pcnAPI.update(id, d) },
        practice:   { create: practiceAPI.create,   update: (id, d) => practiceAPI.update(id, d) },
      };
      if (editTarget) await apiMap[type].update(editTarget._id, form);
      else            await apiMap[type].create(form);
      await fetchAll(true);
      closeModal(type);
  ...
  const handleDelete = async (type, id, confirmMsg, clearSelected = false) => {
    if (!window.confirm(confirmMsg)) return;
    try {
      const delMap = {
        icb: icbAPI.delete, federation: federationAPI.delete,
        pcn: pcnAPI.delete, practice: practiceAPI.delete,
      };
      await delMap[type](id);
      await fetchAll(true);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-slate-200 bg-white shrink-0">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <button onClick={() => fetchAll(true)} title="Refresh"
            className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-all">
      ...
      {error && (
        <div className="mx-5 mt-3 flex items-center gap-2 px-4 py-2.5 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700">
          ...
          <button onClick={() => setError(null)} className="text-red-400 hover:text-red-600 ml-1"><X size={13} /></button>
        </div>
      )}
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden flex-1">
            ...
            <div className="flex-1 overflow-y-auto [scrollbar-width:thin]">
              <HierarchyTree
                ...
                onDeleteICB={id => handleDelete("icb", id, "Delete this ICB?")}
                onEditFederation={f => openModal("federation","edit",f,{name:f.name,icb:f.icb?._id||f.icb||"",type:f.type,notes:f.notes||""})}
                onDeleteFederation={id => handleDelete("federation", id, "Delete this Federation?")}
                ...
                onDeletePCN={id => handleDelete("pcn", id, "Delete this PCN?", selected?.data?._id === id)}
                ...
                onDeletePractice={id => handleDelete("practice", id, "Delete this Practice?", selected?.data?._id === id)}
              />
            </div>
          </div>
        </div>
      ...
      {modals.icb        && <ICBModal        mode={modals.icb}        form={forms.icb}        setForm={v=>setForm("icb",v)}        onSave={()=>handleSave("icb")}        onClose={()=>closeModal("icb")}        saving={saving}/>}
      {modals.federation && <FederationModal mode={modals.federation} form={forms.federation} setForm={v=>setForm("federation",v)} icbs={icbs} onSave={()=>handleSave("federation")} onClose={()=>closeModal("federation")} saving={saving}/>}
      {modals.pcn        && <PCNModal        mode={modals.pcn}        form={forms.pcn}        setForm={v=>setForm("pcn",v)}        icbs={icbs} federations={federations} onSave={()=>handleSave("pcn")} onClose={()=>closeModal("pcn")} saving={saving}/>}
      {modals.practice   && <PracticeModal   mode={modals.practice}   form={forms.practice}   setForm={v=>setForm("practice",v)}   pcns={pcns} onSave={()=>handleSave("practice")} onClose={()=>closeModal("practice")} saving={saving}/>}
    </div>
  );
}
...
function ICBsView() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const { data } = await icbAPI.getAll(); setIcbs(data.icbs || []); }
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if (editTarget) await icbAPI.update(editTarget._id, form);
      else            await icbAPI.create(form);
  ...
  const del = async (id, name) => {
    ...
    try { await icbAPI.delete(id); await load(); }
    catch (err) { alert(err.response?.data?.message || "Delete failed"); }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      <ViewHeader icon={Building2} iconBg="bg-blue-600" title="ICBs"
        ...
        onRefresh={load}
        onAdd={() => { setEditTarget(null); setForm(BLANK.icb); setModal("add"); }}
        addLabel="Add ICB" />
                  ...
                  <tr key={icb._id} className="group hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <IconBtn icon={Pencil} onClick={() => { setEditTarget(icb); setForm({name:icb.name,region:icb.region||"",code:icb.code||"",notes:icb.notes||""}); setModal("edit"); }} hoverClass="hover:bg-blue-50 hover:text-blue-600" />
...
function PCNsView() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [p, i, f] = await Promise.all([pcnAPI.getAll(), icbAPI.getAll(), federationAPI.getAll()]);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if (editTarget) await pcnAPI.update(editTarget._id, form);
      else            await pcnAPI.create(form);
      await load(); setModal(null);
    } catch (err) { alert(err.response?.data?.message || "Save failed"); }
    finally { setSaving(false); }
  };
  ...
  const del = async (id, name) => {
    if (!window.confirm(`Delete "${name}"?`)) return;
    try { await pcnAPI.delete(id); await load(); if (selected?.data?._id === id) setSelected(null); }
    catch (err) { alert(err.response?.data?.message || "Delete failed"); }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      <ViewHeader icon={Network} iconBg="bg-purple-600" title="PCNs"
        ...
        onRefresh={load}
        onAdd={() => { setEditTarget(null); setForm(BLANK.pcn); setModal("add"); }}
        addLabel="Add PCN" />
      <div className="flex flex-1 gap-4 p-5 pt-3 min-h-0 overflow-hidden">
        <div className="w-72 shrink-0 flex flex-col gap-2.5 min-h-0">
          ...
          <select value={filterICB} onChange={e => setFilterICB(e.target.value)}
            className="w-full px-3 py-2 text-xs border border-slate-200 rounded-xl bg-white focus:outline-none focus:border-blue-400 transition-all text-slate-600">
              ...
              filtered.map(pcn => {
                ...
                return (
                  <div key={pcn._id} onClick={() => setSelected({ type: "pcn", data: pcn })}
                    className={`group p-3.5 rounded-xl border cursor-pointer transition-all ${active ? "bg-purple-50 border-purple-200 shadow-sm" : "bg-white border-slate-200 hover:border-slate-300 hover:shadow-sm"}`}>
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="hidden group-hover:flex items-center gap-0.5 shrink-0">
                        <IconBtn icon={Pencil} size={11} onClick={e => { e.stopPropagation(); setEditTarget(pcn); setForm({name:pcn.name,icb:pcn.icb?._id||pcn.icb||"",federation:pcn.federation?._id||pcn.federation||"",federationName:pcn.federationName||"",annualSpend:pcn.annualSpend||"",contractType:pcn.contractType||"",xeroCode:pcn.xeroCode||"",xeroCategory:pcn.xeroCategory||"",notes:pcn.notes||"",contractRenewalDate:pcn.contractRenewalDate?.slice?.(0,10)||"",contractExpiryDate:pcn.contractExpiryDate?.slice?.(0,10)||""}); setModal("edit"); }} hoverClass="hover:bg-blue-50 hover:text-blue-600" />
...
function PracticesView() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [pr, p] = await Promise.all([practiceAPI.getAll(), pcnAPI.getAll()]);
  ...
  const filtered = practices.filter(pr => {
    const q = search.toLowerCase();
    return (!search || pr.name.toLowerCase().includes(q) || (pr.odsCode||"").toLowerCase().includes(q) || (pr.city||"").toLowerCase().includes(q)) &&
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if (editTarget) await practiceAPI.update(editTarget._id, form);
      else            await practiceAPI.create(form);
      await load(); setModal(null);
    } catch (err) { alert(err.response?.data?.message || "Save failed"); }
    finally { setSaving(false); }
  };
  ...
  const del = async (id, name) => {
    if (!window.confirm(`Delete "${name}"?`)) return;
    try { await practiceAPI.delete(id); await load(); if (selected?.data?._id === id) setSelected(null); }
    catch (err) { alert(err.response?.data?.message || "Delete failed"); }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 ">
      <ViewHeader icon={Stethoscope} iconBg="bg-emerald-600" title="Practices / Surgeries"
        ...
        onRefresh={load}
        onAdd={() => { setEditTarget(null); setForm(BLANK.practice); setModal("add"); }}
      ...
      <div className="flex flex-1 gap-4 p-5 pt-3 min-h-0 overflow-hidden">
        <div className="w-72 shrink-0 flex flex-col gap-2.5 min-h-0">
          ...
          <select value={filterPCN} onChange={e => setFilterPCN(e.target.value)}
            className="w-full px-3 py-2 text-xs border border-slate-200 rounded-xl bg-white focus:outline-none focus:border-emerald-400 transition-all text-slate-600">
              ...
              filtered.map(pr => {
                ...
                return (
                  <div key={pr._id} onClick={() => setSelected({ type: "practice", data: pr })}
                    className={`group p-3.5 rounded-xl border cursor-pointer transition-all ${active ? "bg-emerald-50 border-emerald-200 shadow-sm" : "bg-white border-slate-200 hover:border-slate-300 hover:shadow-sm"}`}>
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="hidden group-hover:flex items-center gap-0.5 shrink-0">
                        <IconBtn icon={Pencil} size={11} onClick={e => { e.stopPropagation(); setEditTarget(pr); setForm({name:pr.name,pcn:pr.pcn?._id||pr.pcn||"",odsCode:pr.odsCode||"",address:pr.address||"",city:pr.city||"",postcode:pr.postcode||"",fte:pr.fte||"",contractType:pr.contractType||"",systemAccessNotes:pr.systemAccessNotes||"",xeroCode:pr.xeroCode||"",xeroCategory:pr.xeroCategory||"",notes:pr.notes||""}); setModal("edit"); }} hoverClass="hover:bg-blue-50 hover:text-blue-600" />
...
function HistoryView() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data: pd } = await pcnAPI.getAll();
      const allPCNs = pd.pcns || [];
      setPcns(allPCNs);
      const results = await Promise.all(
        allPCNs.slice(0, 30).map(pcn =>
          historyAPI.get("PCN", pcn._id, { limit: 50 })
  ...
  const toggleStar = async (id) => {
    await historyAPI.toggleStar(id).catch(() => {});
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      <ViewHeader icon={MessageSquare} iconBg="bg-blue-600" title="Contact History"
        subtitle="All communications across PCNs and Practices" onRefresh={load} showAdd={false} />
      <div className="px-5 py-3 space-y-2 border-b border-slate-200 bg-white shrink-0">
        <div className="flex gap-2.5 flex-wrap">
          ...
          <button onClick={() => setStarOnly(s => !s)}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border transition-all ${starOnly ? "bg-amber-50 text-amber-600 border-amber-300" : "bg-white text-slate-500 border-slate-200 hover:bg-slate-50"}`}>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-3 space-y-2 [scrollbar-width:thin]">
        {loading ? <Spinner /> : visible.length === 0 ? <EmptyMsg icon={MessageSquare} msg="No history entries found" /> :
          visible.map(log => {
            ...
            return (
              <div key={log._id} className={`flex items-start gap-3 p-3.5 rounded-xl border transition-all ${log.starred ? "bg-amber-50 border-amber-200" : "bg-white border-slate-200 hover:border-slate-300"}`}>
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${meta.color}`}><Icon size={13} /></div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1 flex-wrap">
                    ...
                    <span className="flex items-center gap-1 text-[11px] text-slate-500"><Network size={9} />{log._entityName}</span>
                    {log.createdBy?.name && <span className="text-[11px] text-slate-400">by {log.createdBy.name}</span>}
                  </div>
                </div>
                <button onClick={() => toggleStar(log._id)}
                  className={`w-6 h-6 rounded-lg flex items-center justify-center transition-all shrink-0 ${log.starred ? "text-amber-500" : "text-slate-300 hover:text-amber-400"}`}>
...
function RestrictedView() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [pd, prd] = await Promise.all([pcnAPI.getAll(), practiceAPI.getAll()]);
      const rows = [];
      for (const pcn of (pd.data.pcns || [])) {
        if (!pcn.restrictedClinicians?.length) continue;
        const full = await pcnAPI.getById(pcn._id).catch(() => null);
      ...
      for (const pr of (prd.data.practices || [])) {
        ...
        const full = await practiceAPI.getById(pr._id).catch(() => null);
    ...
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <div className="px-5 pt-3 pb-2 space-y-2.5">
        <div className="flex items-start gap-2.5 p-3.5 bg-red-50 border border-red-100 rounded-xl">
          ...
          <p className="text-xs text-red-700 leading-relaxed">Clinicians listed here are <strong>blocked from placement</strong> at the linked sites.</p>
        </div>
            ...
            <div key={i} className="bg-white border border-red-200 rounded-xl overflow-hidden shadow-sm">
              ...
              <div className="px-4 py-3 flex flex-wrap gap-1.5">
                {entry.entities.map((ent, j) => {
                  ...
                  return (
                    <span key={j} className="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 border border-slate-200">
                      <EIcon size={10} className={ent.type === "PCN" ? "text-purple-500" : "text-emerald-500"} />
                      {ent.name}<span className="text-[9px] text-slate-400">({ent.type})</span>
                    </span>
                  );
                })}
              </div>
  ...
  return (
    <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 bg-white shrink-0">
      ...
      <div className="flex items-center gap-1.5">
        <button onClick={onRefresh} className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-all">
          <RefreshCw size={13} />
        </button>
        {showAdd && onAdd && (
          <Btn onClick={onAdd} size="sm"><Plus size={12} className="mr-1" />{addLabel}</Btn>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className="w-full pl-8 pr-8 py-2 text-xs border border-slate-200 rounded-xl bg-white placeholder-slate-400 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all" />
      {value && (
...
function IconBtn({ icon: Icon, onClick, hoverClass, size = 13 }) {
  return (
    <button onClick={onClick} className={`w-6 h-6 flex items-center justify-center rounded-lg text-slate-400 transition-colors ${hoverClass}`}>
      <Icon size={size} />
    </button>
  );
}
...
function Spinner() {
  return <div className="flex justify-center py-14"><Loader2 size={22} className="animate-spin text-blue-400" /></div>;
}
...
function SelectPrompt({ icon: Icon, msg }) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center h-full min-h-[340px] text-center p-8">
...
function TypePill({ active, onClick, label }) {
  return (
    <button onClick={onClick} className={`px-2.5 py-1 rounded-full text-[10px] font-bold border transition-all ${active ? "bg-blue-600 text-white border-blue-600" : "bg-white text-slate-500 border-slate-200 hover:bg-slate-50"}`}>
      {label}
    </button>
  );
}