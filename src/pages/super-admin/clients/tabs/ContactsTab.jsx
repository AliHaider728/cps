/**
 * tabs/ContactsTab.jsx
 * Manage contacts for PCN or Practice.
...
export default function ContactsTab({ data, entityType, entityId, onRefresh }) {
  ...
  const saveContacts = async (newContacts) => {
    ...
    try {
      const url = entityType === "PCN"
        ? `${API}/clients/pcn/${entityId}`
        : `${API}/clients/practice/${entityId}`;
      await axios.put(url, { contacts: newContacts });
      onRefresh?.();
    } catch (e) {
  ...
  const handleAdd = async () => {
    ...
    await saveContacts([...contacts, form]);
  ...
  const handleDelete = async (idx) => {
    ...
    await saveContacts(contacts.filter((_, i) => i !== idx));
  };

  const startEdit = (c, idx) => {
    setEditing(idx);
    setForm({ name: c.name || "", role: c.role || "", email: c.email || "", phone: c.phone || "", type: c.type || "general" });
    setAdding(false);
    setErr("");
  };
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        {!showForm && (
          <Btn size="sm" onClick={() => { setAdding(true); setEditing(null); setForm(EMPTY); setErr(""); }}>
      ...
      {showForm && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Field label="Email">
              <Input type="email" value={form.email} onChange={e => set("email", e.target.value)} placeholder="email@nhs.uk" />
            </Field>
            <Field label="Phone">
              <Input value={form.phone} onChange={e => set("phone", e.target.value)} placeholder="01234 567 890" />
            </Field>
            <Field label="Type">
              <select
                value={form.type}
      ...
      {contacts.length === 0 && !showForm ? (
        ...
        <div className="space-y-2.5">
          {contacts.map((c, i) => {
            ...
            return (
              <div key={i}
                className="group flex items-start gap-3 p-4 bg-slate-50 border border-slate-200
                ...
                <div className="hidden group-hover:flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={() => handleDelete(i)}
                    className="w-7 h-7 rounded-lg flex items-center justify-center
                      text-slate-400 hover:bg-red-50 hover:text-red-600 transition-colors">
                    <Trash2 size={12} />
                  </button>
                </div>
              </div>
