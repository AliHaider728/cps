  ...
  return (
    <>
      ...
      <Section title="Onboarding Checklist">
        ...
        {/* Compliance score */}
        {(() => {
          const keys = ["ndaSigned","dsaSigned","mouReceived","welcomePackSent",
          ...
          const done  = keys.filter(k => d[k]).length;
          const pct   = Math.round((done / keys.length) * 100);
          const color = pct >= 80 ? "bg-emerald-500" : pct >= 50 ? "bg-amber-500" : "bg-red-500";
          return (
            <div className="mt-3">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-semibold text-slate-500">Onboarding Progress</span>
                <span className="text-xs font-bold text-slate-700">{done}/{keys.length} ({pct}%)</span>
              </div>
...
function CompliancePill({ label, done }) {
  return (
    <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-xs font-semibold
      ...
      <span className={`w-2 h-2 rounded-full shrink-0 ${done ? "bg-emerald-500" : "bg-slate-300"}`} />
      {label}
    </div>
  );
}
