 ...
 * Structured system access management per Practice.
 * From spec section 4: Auto-generate system access request emails,
 * track status (requested/pending/granted), log to contact history.
 */
import { useState } from "react";
import axios from "axios";
import {
  Monitor, CheckCircle2, Clock, AlertCircle, Send, Plus, Loader2, X
} from "lucide-react";
...
export default function SystemAccessTab({ data, entityType, entityId, onRefresh }) {
  ...
  const sendRequest = async () => {
    ...
    setSending(true);
    try {
      await axios.post(`${API}/clients/${entityType}/${entityId}/system-access-request`, {
        systems: reqForm.systems,
        clinicianDetails: {
          name: reqForm.name, gphcNumber: reqForm.gphcNumber,
          smartCardNumber: reqForm.smartCardNumber, email: reqForm.email, phone: reqForm.phone,
        },
        notes: reqForm.notes,
      });
      setSent(true);
      onRefresh?.();
      setTimeout(() => { setSent(false); setShowRequest(false); }, 2000);
    } catch (err) {
  ...
  // Parse free-text notes to detect systems if no structured data
    ...
    <div className="p-6 space-y-5">
          ...
          {sent ? (
            ...
            <>
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Btn size="sm" onClick={sendRequest} disabled={sending || !reqForm.name || !reqForm.systems.length}>
              ...
              return (
                <div key={i} className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  ...
                  <div className="flex-1">
                    ...
                    {sa.notes && <p className="text-xs text-slate-500">{sa.notes}</p>}
                  </div>
                  <span className={`flex items-center gap-1.5 text-[11px] font-bold
                    px-2.5 py-1 rounded-full ${conf.color}`}>
                    <Icon size={10} />
                    {conf.label}
                  </span>
                </div>
      ...
      {!systemAccess.length && !detected.length && !data?.systemAccessNotes && (
        <div className="text-center py-10">
          ...
          <p className="text-sm text-slate-400">No system access records</p>
