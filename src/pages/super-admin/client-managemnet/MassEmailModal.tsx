...
export default function MassEmailModal({ entityType, entityId, contacts = [], onClose }: MassEmailModalProps) {
  const [subject,    setSubject]    = useState<string>("");
  const [body,       setBody]       = useState<string>("");
  const [recipients, setRecipients] = useState<Recipient[]>(
    contacts.filter(c => c.email).map(c => ({ name: c.name || "", email: c.email || "", selected: true }))
  );
  const [custom, setCustom] = useState<{ name: string; email: string }>({ name: "", email: "" });
  const [sent,   setSent]   = useState<boolean>(false);

  // ✅ React Query
  const sendEmail = useSendMassEmail();

  const toggle    = (i: number) => setRecipients(prev => prev.map((r, idx) => idx === i ? {...r, selected: !r.selected} : r));
  const addCustom = () => {
