...
export default function MassEmailModal({ entityType, entityId, contacts = [], onClose }) {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      await sendMassEmail(entityType, entityId, { subject, body, recipients: valid });
      setSent(true);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl border border-slate-200 max-h-[90vh] flex flex-col">
          ...
          <div>
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden">
                ...
                <div className="divide-y divide-slate-100 max-h-36 overflow-y-auto [scrollbar-width:thin]">
                  {recipients.map((r, i) => (
                    <label key={i} className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 cursor-pointer">
                      ...
                      <button onClick={(e) => { e.preventDefault(); setRecipients(p => p.filter((_, idx) => idx !== i)); }}
              ...
              <div className="px-4 py-3 bg-slate-50 border-t border-slate-100 flex gap-2">
                ...
                <button
                  onClick={addCustom}
                  className="px-2.5 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-all"
                >
                  <Plus size={13} />
                </button>
              </div>
        ...
        <div className="px-6 pb-5 pt-3 border-t border-slate-100 flex gap-3 shrink-0">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-[13px] font-semibold text-slate-600 hover:bg-slate-50 transition-all">
            Cancel
          </button>
          <button
            onClick={handleSend}
            disabled={sending || sent || !subject.trim() || !body.trim() || recipients.filter(r => r.selected).length === 0}
            className={`flex-1 py-2.5 rounded-xl text-white text-[13px] font-semibold flex items-center justify-center gap-2 transition-all
              ${sent ? "bg-green-500" : "bg-blue-600 hover:bg-blue-700 disabled:opacity-50"}`}
          >
            {sent ? (
              <><Check size={14} /> Sent!</>
            ) : sending ? (
