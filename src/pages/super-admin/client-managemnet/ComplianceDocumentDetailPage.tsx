/**
 * ComplianceDocumentDetailPage.tsx
...
const Spinner = ({ cls = "border-white" }: SpinnerProps) => (
...
const ActivePill = ({ active }: ActivePillProps) => (
...
const BoolBadge = ({ ok, trueLabel, falseLabel }: BoolBadgeProps) => (
...
export function ComplianceDocumentDetailPage() {
  ...
  const set = (k: keyof ComplianceForm) => (v: any) => setForm(f => ({ ...f, [k]: v }));
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              {[
                ...
                <div key={label as string} className="flex justify-between py-2.5 border-b border-slate-50 last:border-0">
            ...
            <div className="space-y-3">
              {[["Mandatory",doc.mandatory],["Expirable",doc.expirable],["Active",doc.active]].map(([label, val]) => (
                <div key={label as string} className="flex justify-between items-center py-2.5 border-b border-slate-50 last:border-0">
                  <span className="text-sm text-slate-500">{label as string}</span>
                  {val
