 ...
 * Uses: GET/POST/DELETE /:entityType/:entityId/reporting-archive
 */
import { useState, useRef } from "react";
import {
  Archive, Upload, Trash2, Download, Star, X, Check, Plus,
...
/* ── Upload modal ── */
const UploadModal = ({ entityType, entityId, onClose, onSave, saving }) => {
  const now = new Date();
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md flex flex-col">
        ...
        <div className="flex gap-3 px-6 pb-5">
          ...
          <button onClick={handle} disabled={saving || !file}
            ...
            {saving ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/> : <Upload size={14} />}
            Upload Report
          </button>
        </div>
      </div>
...
export default function ReportingArchivePanel({ entityType, entityId }) {
  ...
  const filtered = filterYear
    ? archive.filter(r => String(r.year) === filterYear)
    : archive;
  ...
  const handleUpload = async (formData) => {
    await addReport.mutateAsync(formData);
    setShowUpload(false);
  };
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">{archive.length} report{archive.length !== 1 ? "s" : ""} stored</p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-14 text-slate-400">
            ...
            <p className="text-sm font-semibold">{archive.length === 0 ? "No reports uploaded yet" : "No reports for selected year"}</p>
            {archive.length === 0 && (
              <button onClick={() => setShowUpload(true)} className="mt-3 text-blue-600 text-xs font-bold hover:underline">Upload the first report</button>
            )}
          </div>
        ) : (
          <div className="grid gap-2.5">
            {filtered.map((report, idx) => {
              ...
              return (
                <div key={report._id || idx} className="flex items-center gap-4 px-4 py-3.5 bg-slate-50 rounded-xl border border-slate-200 group hover:border-blue-200 hover:bg-white transition-all">
