...
interface BadgeProps {
  children: ReactNode;
  tone?: "slate" | "blue" | "purple" | "teal" | "indigo" | "green";
}
...
const PracticeRow: React.FC<PracticeRowProps> = ({ practice, navigate, query }) => (
  <button
    onClick={() =>
...
const PCNCard: React.FC<PCNCardProps> = ({ pcn, navigate, query, defaultOpen = false }) => {
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-0.5 flex-wrap">
            ...
            {pcn.annualSpend && pcn.annualSpend > 0 ? (
...
const ICBSection: React.FC<ICBSectionProps> = ({ icb, navigate, query, defaultOpen = true }) => {
  const [open, setOpen] = useState(defaultOpen);
...
const SearchResults: React.FC<SearchResultsProps> = ({ results, navigate, onSelect, query }) => {
  const typeIcon: Record<string, React.ElementType> = { icb: Building2, pcn: Network, practice: Stethoscope };
  const typeColor: Record<string, string> = {
    icb: "bg-blue-50 text-blue-600",
    pcn: "bg-purple-50 text-purple-600",
    practice: "bg-teal-50 text-teal-600",
  };
  const getPath = (r: SearchResult) => `/dashboard/super-admin/clients/${r._type}/${r._id}`;
  ...
  const localResults = useMemo(() => {
    ...
    for (const icb of tree) {
      if (
        ...
        out.push({ ...icb, _type: "icb" } as SearchResult);
      }
      for (const pcn of icb.pcns || []) {
        if (
          ...
          out.push({ ...pcn, _type: "pcn" } as SearchResult);
        }
        for (const pr of pcn.practices || []) {
          if (
            ...
            out.push({ ...pr, _type: "practice" } as SearchResult);
          }
        }
      }
    }
