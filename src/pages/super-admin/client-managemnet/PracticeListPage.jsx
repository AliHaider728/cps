...
export default function PracticeListPage() {
  ...
  const handleDelete = async (p) => {
    ...
    try { await deletePractice.mutateAsync(p._id); } catch (e) { alert(e.message); }
  };
    ...
    <div>
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs text-slate-400 mb-1">
            ...
            <ChevronRight size={12} /><span className="text-slate-600 font-medium">Practices</span>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-16 flex flex-col items-center text-center">
          <Stethoscope size={36} className="text-slate-300 mb-3" /><p className="text-slate-500 font-semibold">No practices found</p>
        </div>
      ) : (
        <div className="grid gap-3">
          {practices.map(p => (
            <div key={p._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-teal-200 transition-all p-5 flex items-center gap-4 group">
              <div className="w-11 h-11 rounded-xl bg-teal-600 flex items-center justify-center shrink-0"><Stethoscope size={18} className="text-white" /></div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1 flex-wrap">
                  ...
                  {p.pcn?.name && <span className="text-[11px] text-slate-400 flex items-center gap-1"><Network size={10}/> {p.pcn.name}</span>}
              ...
              <div className="flex items-center gap-2 shrink-0">
                {p.contractType && <span className="text-[10px] bg-teal-50 text-teal-700 font-bold px-1.5 py-0.5 rounded-md">{p.contractType}</span>}
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => navigate(`/dashboard/super-admin/clients/practice/${p._id}`)} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-teal-600 hover:bg-teal-50 transition-all"><Eye size={14}/></button>
                  <button onClick={() => setModal(p)} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-all"><Edit2 size={14}/></button>
                  <button onClick={() => handleDelete(p)} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all"><Trash2 size={14}/></button>
                </div>
              </div>
            </div>
          ))}
