    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/50 p-0 sm:p-4 backdrop-blur-sm">
      <div className="flex w-full sm:max-w-lg max-h-[95dvh] sm:max-h-[90vh] flex-col rounded-t-2xl sm:rounded-2xl border border-slate-200 bg-white shadow-2xl">
        ...
        <div className="flex shrink-0 gap-3 border-t border-slate-100 px-5 pb-6 pt-3 sm:pb-4">
          <button onClick={onClose} className="flex-1 rounded-xl border border-slate-200 py-2.5 text-[13px] font-semibold text-slate-600 hover:bg-slate-50">
            Cancel
          </button>
...
export default function PracticeListPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      id: "group",
      hideOnMobile: true,
      render: (p) => p.complianceGroup?.name
    ...
    },
    {
      ...
      id: "patients",
      hideOnMobile: true,
      render: (p) => p.patientListSize ? Number(p.patientListSize).toLocaleString() : "—",
      cellClassName: "px-4 py-3 whitespace-nowrap text-slate-600 align-top",
    },
    {
      header: "Actions",
    ...
    <div className="space-y-5 px-2 sm:px-0">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-slate-800">Practices / Surgeries</h1>
          <p className="mt-1 text-sm text-slate-500">
            {filteredPractices.length} visible of {practices.length} practices
          </p>
        </div>
        <button onClick={() => setModal("add")}
          className="flex items-center gap-2 rounded-xl bg-teal-600 px-4 py-2.5 text-[13px] font-semibold text-white hover:bg-teal-700 transition-colors">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <FilterChip label="City">
            <input value={filters.city} onChange={(e) => setFilters((c) => ({ ...c, city: e.target.value }))}
              placeholder="Any" className="w-20 bg-transparent text-sm outline-none" />
          </FilterChip>
