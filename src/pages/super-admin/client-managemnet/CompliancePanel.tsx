...
interface FilterBarProps {
  ...
  filterOptions: { key: string; label: string; options: [string, string][] }[];
  searchPlaceholder?: string;
}
const FilterBar: React.FC<FilterBarProps> = ({ search, setSearch, filters, setFilters, filterOptions, searchPlaceholder }) => (
...
interface GroupModalProps {
  ...
  allDocs: any[];
  onClose: () => void;
  onSave: (form: DocumentGroupForm) => Promise<void>;
}

/* ── GroupModal — UPDATED: +applicableContractTypes, +colour, +notes ── */
const GroupModal: React.FC<GroupModalProps> = ({ existing, allDocs, onClose, onSave }) => {
  ...
  const [form, setForm] = useState<DocumentGroupForm>({
    ...
    ...(existing
      ? {
          ...existing,
          documents: (existing.documents || []).map((d: any) => d._id || d),
...
const GROUP_FILTER_OPTIONS: { key: string; label: string; options: [string, string][] }[] = [
...
const GroupListTab: React.FC<GroupListTabProps> = ({ groups, loading, onAdd, onEdit, onDelete, navigate }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (grp: any) => grp.displayOrder,
    ...
    },
    {
      ...
      render: (grp: any) => <ActivePill active={grp.active} />,
    },
    {
      header: "", id: "actions",
      render: (grp: any) => (
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        ...
        getRowClassName={(grp: any) => `${!grp.active ? "opacity-60" : ""} hover:bg-slate-50 transition-colors`}
      />
    </div>
  );
};
