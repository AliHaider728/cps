...
const FedModal = ({ existing, icbs, onClose, onSave }) => {
  const [form, setForm] = useState({
    ...
    type: existing?.type || "federation", notes: existing?.notes || "",
  });
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-200">
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} placeholder="e.g. Salford Together Federation"
            ...
            <div className="grid grid-cols-3 gap-2">
              {["federation","INT","other"].map(t => (
                <button key={t} onClick={() => setForm(f => ({...f, type: t}))}
                  className={`py-2.5 rounded-xl border text-sm font-semibold capitalize transition-all ${form.type === t ? "bg-indigo-50 border-indigo-300 text-indigo-700" : "bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100"}`}>
                  {t}
                </button>
              ))}
            </div>
...
export default function FederationListPage() {
  ...
  const { data: fedData, isLoading } = useFederations(icbFilter || undefined);
  ...
  return (
    <div>
        ...
        <div className="grid gap-3">
          {feds.map(fed => (
            <div key={fed._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all p-5 flex items-center gap-4 group">
              <div className="w-12 h-12 rounded-xl bg-indigo-500 flex items-center justify-center shrink-0"><Layers size={20} className="text-white" /></div>
              ...
              <div className="flex items-center gap-2 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => setModal(fed)} className="w-9 h-9 rounded-lg flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all"><Edit2 size={15}/></button>
                <button onClick={() => handleDelete(fed)} className="w-9 h-9 rounded-lg flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all"><Trash2 size={15}/></button>
              </div>
            </div>
          ))}
        </div>
      ...
      {modal && (
        <FedModal existing={modal === "add" ? null : modal} icbs={icbs} onClose={() => setModal(null)} onSave={handleSave} />
      )}
    </div>
  );
}
