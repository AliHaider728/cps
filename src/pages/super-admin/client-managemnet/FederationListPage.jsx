...
export default function FederationListPage() {
  ...
  const handleDelete = async (fed) => {
    ...
    try { 
      await deleteFed.mutateAsync(fed._id); 
    } catch (e) { 
      alert(e.message); 
    }
  };
      ...
      {isLoading ? (
        ...
        <div className="grid gap-3">
          {feds.map(fed => (
            <div 
              key={fed._id} 
              className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all p-5 flex items-center gap-4 group"
            >
