    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h3 className="text-[15px] font-bold text-slate-800">{existing ? "Edit Federation" : "Add Federation / INT"}</h3>
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <select value={form.icb} onChange={e => setForm(f => ({...f, icb: e.target.value}))}
              className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-[13px] bg-slate-50 focus:outline-none focus:border-blue-400 focus:bg-white transition-all cursor-pointer">
            ...
            <div className="grid grid-cols-3 gap-2">
              {["federation","INT","other"].map(t => (
                <button key={t} onClick={() => setForm(f => ({...f, type: t}))}
                  className={`py-2 rounded-xl border text-[12px] font-semibold capitalize transition-all
        ...
        <div className="flex gap-3 px-6 pb-5">
          ...
          <button onClick={handle} disabled={saving}
            className="flex-1 py-2.5 rounded-xl bg-indigo-600 text-white text-[13px] font-semibold hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center justify-center gap-2">
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setModal("add")}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 text-white text-[13px] font-semibold hover:bg-indigo-700 transition-all">
            <Plus size={14}/> Add Federation
          </button>
        </div>
      </div>
                ...
                <div className="flex items-center gap-3 mt-1 flex-wrap">
                  {fed.icb?.name && (
                    <span className="text-[11px] text-slate-400 flex items-center gap-1">
                      <Building2 size={10}/> {fed.icb.name}
                    </span>
                  )}
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md border capitalize ${TYPE_STYLE[fed.type] || TYPE_STYLE.other}`}>
                    {fed.type}
                  </span>
                </div>
              </div>
