...
const ICBModal = ({ existing, onClose, onSave }) => {
  const [form, setForm] = useState({ name: existing?.name || "", region: existing?.region || "", code: existing?.code || "", notes: existing?.notes || "" });
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-200">
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} placeholder="e.g. NHS Greater Manchester ICB"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.region} onChange={e => setForm(f => ({...f, region: e.target.value}))} placeholder="North West"
                className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-800 bg-slate-50 focus:outline-none focus:border-blue-400 focus:bg-white transition-all" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">Code</label>
              <input value={form.code} onChange={e => setForm(f => ({...f, code: e.target.value}))} placeholder="QOP"
                className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-800 bg-slate-50 focus:outline-none focus:border-blue-400 focus:bg-white transition-all" />
            </div>
          </div>
          <div>
...
export function ICBListPage() {
  ...
  return (
    <div>
        ...
        <div className="grid gap-3">
          {icbs.map(icb => (
            <div key={icb._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-200 transition-all p-5 flex items-center gap-4 group">
              <div className="w-12 h-12 rounded-xl bg-blue-600 flex items-center justify-center shrink-0"><Building2 size={20} className="text-white" /></div>
              <div className="flex-1 min-w-0">
                <p className="text-base font-bold text-slate-800">{icb.name}</p>
                <div className="flex items-center gap-4 mt-1.5 flex-wrap">
                  {icb.region && <span className="text-sm text-slate-400 flex items-center gap-1"><MapPin size={11} />{icb.region}</span>}
                  {icb.code   && <span className="text-sm text-slate-400 flex items-center gap-1"><Hash size={11} />{icb.code}</span>}
                  {icb.notes  && <span className="text-sm text-slate-400 flex items-center gap-1 truncate max-w-[200px]"><FileText size={11} />{icb.notes}</span>}
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={() => setModal(icb)}
                ...
                <button onClick={() => handleDelete(icb)}
      ...
      {modal && (
        <ICBModal existing={modal === "add" ? null : modal} onClose={() => setModal(null)} onSave={handleSave} />
      )}
    </div>
  );
}
