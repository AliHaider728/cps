 ...
 * Route: /dashboard/super-admin/compliance/documents/:id
 */
import { useState, useEffect, useCallback } from "react";
import { useParams, useNavigate } from "react-router-dom";
import {
  FileText, ArrowLeft, Edit2, Trash2, Check,
  ChevronRight, CheckCircle2, XCircle, Layers, Clock
} from "lucide-react";
import {
...
export default function ComplianceDocumentDetailPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getComplianceDocById(id);
      setDoc(res.doc);
      setGroups(res.groups || []);
      setForm({
        name:                res.doc.name,
        displayOrder:        res.doc.displayOrder,
        mandatory:           res.doc.mandatory,
        expirable:           res.doc.expirable,
        active:              res.doc.active,
        defaultExpiryDays:   res.doc.defaultExpiryDays,
        defaultReminderDays: res.doc.defaultReminderDays,
      });
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await updateComplianceDoc(id, form);
      setDoc(res.doc);
      setEditing(false);
  ...
  const handleDelete = async () => {
    ...
    try { await deleteComplianceDoc(id); navigate(-1); }
    catch (e) { alert(e.message); }
  };
        ...
        <div className="flex flex-wrap items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-2 flex-wrap">
              ...
              {doc.expirable && (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded border bg-amber-50 text-amber-700 border-amber-200 flex items-center gap-1">
                  <Clock size={9} /> Expirable
                </span>
              )}
            </div>
          </div>
