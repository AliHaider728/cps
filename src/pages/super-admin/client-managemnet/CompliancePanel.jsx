/**
 * CompliancePanelEnhanced.jsx — React Query version
 * Props: entityType, entity, fieldSaving, onToggle, onPatch, practiceRollup
 */
...
import {
  ...
  Search, Clock, Loader2, ExternalLink
} from "lucide-react";
import {
  useComplianceDocs, useCreateComplianceDoc, useUpdateComplianceDoc, useDeleteComplianceDoc,
  useDocumentGroups, useCreateDocumentGroup, useUpdateDocumentGroup, useDeleteDocumentGroup,
...
const DocModal = ({ existing, onClose, onSave }) => {
  ...
  const [form, setForm] = useState({ name:"", displayOrder:0, mandatory:true, expirable:false, active:true, defaultExpiryDays:365, defaultReminderDays:28, ...(existing || {}) });
...
const GroupModal = ({ existing, allDocs, onClose, onSave }) => {
  const isEdit = !!existing?._id;
  const [form, setForm] = useState({ name:"", displayOrder:0, active:true, documents:[],
...
const EntityComplianceTab = ({ entity, entityType, groups, docs, fieldSaving, onToggle, practiceRollup }) => {
  ...
  const rollupPct = practiceRollup?.length
    ? Math.round(practiceRollup.reduce((s,p) => { const pd=ONBOARDING_KEYS.filter(c=>p[c.key]).length; return s+(pd/total)*100; }, 0)/practiceRollup.length)
    : null;
          ...
          {ONBOARDING_KEYS.map(c => {
            ...
            return (
              <button key={c.key} onClick={() => onToggle(c.key)} disabled={saving}
                ...
                {saving ? <Spinner cls="border-slate-400" /> : ok ? <CheckCircle2 size={16} className="text-green-600 shrink-0" /> : <XCircle size={16} className="text-slate-400 shrink-0" />}
...
const GroupListTab = ({ groups, allDocs, loading, onAdd, onEdit, onDelete, navigate }) => {
  ...
  const filtered = groups.filter(g=>g.name.toLowerCase().includes(search.toLowerCase())).sort((a,b)=>a.displayOrder-b.displayOrder||a.name.localeCompare(b.name));
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-4 py-3 bg-slate-50 border-b border-slate-200"><span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Document Groups</span></div>
        <table className="w-full text-sm">
          <thead><tr className="bg-white border-b border-slate-200">{["Group Name","Display Order","Active",""].map((h,i)=><th key={i} className="px-4 py-3 text-left text-xs font-bold text-slate-700">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-slate-100">
            {loading ? <tr><td colSpan={4} className="text-center py-12"><Loader2 size={20} className="animate-spin mx-auto text-slate-400" /></td></tr>
              : filtered.length===0 ? <tr><td colSpan={4} className="text-center py-12 text-slate-400 text-sm">No groups found</td></tr>
              : filtered.map(grp => (
...
/* ══════ TAB 3 — DOCS LIST ══════════════════════════════════ */
const DocumentListTab = ({ docs, loading, onAdd, onEdit, onDelete, navigate }) => {
  const [search, setSearch] = useState("");
  const filtered = docs.filter(d=>d.name.toLowerCase().includes(search.toLowerCase())).sort((a,b)=>a.displayOrder-b.displayOrder||a.name.localeCompare(b.name));
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="relative flex-1 max-w-sm">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-4 py-3 bg-slate-50 border-b border-slate-200"><span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Compliance Document / Training List</span></div>
        <table className="w-full text-sm">
          <thead><tr className="bg-white border-b border-slate-200">{["Document Name","Display Order","Mandatory","Expirable","Active",""].map((h,i)=><th key={i} className="px-4 py-3 text-left text-xs font-bold text-slate-700">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-slate-100">
            {loading ? <tr><td colSpan={6} className="text-center py-12"><Loader2 size={20} className="animate-spin mx-auto text-slate-400" /></td></tr>
              : filtered.length===0 ? <tr><td colSpan={6} className="text-center py-12 text-slate-400 text-sm">No documents found</td></tr>
              : filtered.map(doc => (
...
export default function CompliancePanelEnhanced({ entityType="PCN", entity, fieldSaving={}, onToggle, onPatch, practiceRollup=[] }) {
  ...
  // ✅ React Query — auto cached, no manual load needed
  ...
  const deleteDocMut = useDeleteComplianceDoc();
  ...
  const handleSaveDoc = async (form) => {
    if (form._id) await updateDoc.mutateAsync({ id: form._id, data: form });
  ...
  return (
    <div className="space-y-4">
          ...
          {SUBTABS.map(t => {
            ...
            return (
              <button key={t.id} onClick={() => setSubTab(t.id)}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold whitespace-nowrap transition-all shrink-0 ${subTab===t.id?"bg-blue-600 text-white shadow-sm":"text-slate-500 hover:bg-slate-50 hover:text-slate-700"}`}>
      ...
      {subTab==="groups" && (
        <GroupListTab groups={groups} allDocs={activeDocs} loading={groupsLoading}
          onAdd={()=>setGroupModal({})} onEdit={g=>setGroupModal(g)}
          onDelete={handleDeleteGroup} navigate={navigate} />
      )}
      {subTab==="documents" && (
        <DocumentListTab docs={docs} loading={docsLoading}
          onAdd={()=>setDocModal({})} onEdit={d=>setDocModal(d)}
          onDelete={handleDeleteDoc} navigate={navigate} />
      )}

      {docModal!==null   && <DocModal   existing={docModal?._id   ? docModal   : null} onClose={()=>setDocModal(null)}   onSave={handleSaveDoc}   />}
      {groupModal!==null && <GroupModal existing={groupModal?._id ? groupModal : null} allDocs={activeDocs} onClose={()=>setGroupModal(null)} onSave={handleSaveGroup} />}
    </div>
  );
}