...
const DocModal = ({ existing, onClose, onSave }) => {
  ...
  const [form, setForm] = useState({
    name: "", displayOrder: 0, mandatory: true,
    expirable: false, active: true,
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-lg flex flex-col max-h-[92vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          <h3 className="text-[15px] font-bold text-slate-800">
            {isEdit ? "Edit Compliance Document" : "Add New Compliance Document"}
          </h3>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 [scrollbar-width:thin]">
          ...
          <div>
            ...
            <input value={form.name} onChange={e => set("name")(e.target.value)}
              placeholder="e.g. DBS Check/Update Service"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" value={form.defaultExpiryDays}
                onChange={e => set("defaultExpiryDays")(Number(e.target.value))}
                className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm bg-slate-50 focus:outline-none focus:border-blue-400 focus:bg-white transition-all" />
            </div>
          </div>
          ...
          <div>
            ...
            <input type="number" value={form.defaultReminderDays}
              onChange={e => set("defaultReminderDays")(Number(e.target.value))}
              className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm bg-slate-50 focus:outline-none focus:border-blue-400 focus:bg-white transition-all" />
          </div>
...
const GroupModal = ({ existing, allDocs, onClose, onSave }) => {
  ...
  const [form, setForm] = useState({
    ...
    ...(existing
      ? { ...existing, documents: (existing.documents || []).map(d => d._id || d) }
      : {}),
  });
  const [saving, setSaving] = useState(false);
  const [err, setErr] = useState("");
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl flex flex-col max-h-[92vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          <h3 className="text-[15px] font-bold text-slate-800">
            {isEdit ? "Edit Document Group" : "Add New Document Group"}
          </h3>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:bg-slate-100 transition-all">
            <X size={16} />
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5 [scrollbar-width:thin]">
          ...
          <div className="border border-slate-200 rounded-xl overflow-hidden">
            ...
            <div className="p-4">
              ...
              {allDocs.length === 0 && (
                <p className="text-xs text-slate-400 text-center py-4">No active documents available. Create documents first.</p>
              )}
            </div>
          </div>
        </div>
...
const EntityComplianceTab = ({ entity, entityType, groups, docs, fieldSaving, onToggle, practiceRollup }) => {
  ...
  const rollupPct = practiceRollup?.length
    ? Math.round(practiceRollup.reduce((sum, p) => {
        const pd = ONBOARDING_KEYS.filter(c => p[c.key]).length;
        return sum + (pd / total) * 100;
      ...
      <div className={`grid grid-cols-1 ${rollupPct !== null ? "sm:grid-cols-2" : ""} gap-4`}>
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h3 className="text-sm font-bold text-slate-700">{entityType} Onboarding Status</h3>
              <p className="text-xs text-slate-400 mt-0.5">{done} of {total} items complete</p>
            </div>
        ...
        {rollupPct !== null && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-sm font-bold text-slate-700">Practices Roll-up</h3>
                <p className="text-xs text-slate-400 mt-0.5">{practiceRollup.length} practices averaged</p>
              </div>
              <span className={`text-3xl font-bold ${rollupPct === 100 ? "text-green-600" : rollupPct >= 60 ? "text-amber-500" : "text-red-500"}`}>
                {rollupPct}%
              </span>
            </div>
            <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
              <div className={`h-full rounded-full transition-all duration-700 ${rollupPct === 100 ? "bg-green-500" : rollupPct >= 60 ? "bg-amber-500" : "bg-red-500"}`}
      ...
      {groups.filter(g => g.active).length > 0 && (
        <div className="space-y-2">
          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
            <Layers size={13} /> Document Groups
          </h3>
          {groups.filter(g => g.active).map(grp => {
            ...
            return (
              <div key={grp._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <button onClick={() => toggleGroup(grp._id)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">{grpDocs.length} document{grpDocs.length !== 1 ? "s" : ""}</p>
                  </div>
                ...
                {open && grpDocs.length > 0 && (
                  <div className="px-5 pb-4 border-t border-slate-100">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-3">
                      {grpDocs.map(doc => (
                        <div key={doc._id}
                          className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border text-xs font-medium
                            ${doc.active ? "bg-green-50 border-green-200 text-green-700" : "bg-slate-50 border-slate-200 text-slate-500"}`}>
                          ...
                          <div className="flex gap-1 shrink-0">
                            ...
                            {doc.expirable && (
                              <span className="bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded text-[9px] font-bold flex items-center gap-0.5">
                                <Clock size={8} />EXP
                              </span>
                            )}
                          </div>
...
══════════════════════════════════════════════ */
const GroupListTab = ({ groups, allDocs, loading, onAdd, onEdit, onDelete }) => {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr key={grp._id} className={`hover:bg-slate-50 transition-colors group ${!grp.active ? "opacity-60" : ""}`}>
                <td className="px-4 py-3 font-medium text-slate-800">{grp.name}</td>
        ...
        <div className="px-4 py-3 border-t border-slate-100 bg-slate-50">
          <p className="text-xs text-slate-500">Total Records : {filtered.length}</p>
        </div>
      </div>
    </div>
  );
...
══════════════════════════════════════════════ */
const DocumentListTab = ({ docs, loading, onAdd, onEdit, onDelete }) => {
  const [search, setSearch] = useState("");
  const filtered = docs
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button onClick={onAdd}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition-all">
          <Plus size={14} /> Add New Compliance Document
        </button>
      </div>
...
export default function CompliancePanelEnhanced({
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      console.error("CompliancePanelEnhanced load error:", e.message);
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <div className="space-y-4">
      ...
      {subTab === "compliance" && (
        <EntityComplianceTab
          ...
          fieldSaving={fieldSaving} onToggle={onToggle}
          practiceRollup={practiceRollup}
        />
      )}

      {subTab === "groups" && (
        <GroupListTab
          groups={groups} allDocs={activeDocs} loading={loading}
          onAdd={() => setGroupModal({})}
