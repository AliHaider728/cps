...
import { Network, Plus, Eye, Edit2, Trash2, X, Check, ChevronRight, Building2, Layers } from "lucide-react";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-slate-200 max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1 [scrollbar-width:thin]">
          ...
          <div className="grid grid-cols-2 gap-3">
            <F label="ICB *">
              <select value={form.icb} autoComplete="off" onChange={e => setForm(f => ({...f, icb: e.target.value, federation: ""}))}
            ...
            <F label="Federation / INT">
              <select value={form.federation} autoComplete="off" onChange={e => setForm(f => ({...f, federation: e.target.value}))}
                className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm bg-slate-50 focus:outline-none focus:border-blue-400 transition-all cursor-pointer">
...
export default function PCNListPage() {
  ...
  // ✅ React Query — auto refetch when icbFilter changes
  const { data: pcnData, isLoading } = usePCNs(icbFilter ? { icb: icbFilter } : {});
  ...
  const handleDelete = async (pcn) => {
    ...
    try { await deletePCN.mutateAsync(pcn._id); } catch (e) { alert(e.message); }
  };

  const CONTRACT_COLOR = {
    ARRS: "bg-blue-50 text-blue-700 border-blue-200", EA: "bg-green-50 text-green-700 border-green-200",
    Direct: "bg-amber-50 text-amber-700 border-amber-200", Mixed: "bg-purple-50 text-purple-700 border-purple-200",
  };
    ...
    <div>
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-sm text-slate-400 mb-1">
            ...
            <ChevronRight size={13} /><span className="text-slate-600 font-medium">PCNs</span>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-16 flex flex-col items-center text-center">
          <Network size={36} className="text-slate-300 mb-3" /><p className="text-slate-500 font-semibold">No PCNs found</p>
        </div>
      ) : (
        <div className="grid gap-3">
          {pcns.map(pcn => (
            <div key={pcn._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-purple-200 transition-all p-5 flex items-center gap-4 group">
              <div className="w-12 h-12 rounded-xl bg-purple-600 flex items-center justify-center shrink-0"><Network size={20} className="text-white" /></div>
              <div className="flex-1 min-w-0">
                <p className="text-base font-bold text-slate-800">{pcn.name}</p>
                <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                  {pcn.icb?.name        && <span className="text-sm text-slate-400 flex items-center gap-1"><Building2 size={11}/> {pcn.icb.name}</span>}
                  {pcn.federation?.name && <span className="text-sm text-slate-400 flex items-center gap-1"><Layers size={11}/> {pcn.federation.name}</span>}
                  {pcn.annualSpend > 0  && <span className="text-sm text-green-600 font-semibold">£{pcn.annualSpend.toLocaleString()}</span>}
                  {pcn.xeroCode         && <span className="text-sm text-slate-400">Xero: {pcn.xeroCode}</span>}
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                {pcn.contractType && <span className={`text-xs font-bold px-2.5 py-1 rounded-lg border ${CONTRACT_COLOR[pcn.contractType] || "bg-slate-50 text-slate-600 border-slate-200"}`}>{pcn.contractType}</span>}
      ...
      {modal && <PCNModal key={modal === "add" ? "pcn-add" : `pcn-${modal._id}`} existing={modal === "add" ? null : modal} icbs={icbs} federations={feds} groups={groups} onClose={() => setModal(null)} onSave={handleSave} />}
    </div>
  );
}
