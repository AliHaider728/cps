    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-slate-200 max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          <h3 className="text-[15px] font-bold text-slate-800">{existing ? "Edit PCN" : "Add PCN"}</h3>
        ...
        <div className="flex gap-3 px-6 pb-5 pt-3 border-t border-slate-100 shrink-0">
          ...
          <button onClick={handle} disabled={saving}
            className="flex-1 py-2.5 rounded-xl bg-purple-600 text-white text-[13px] font-semibold hover:bg-purple-700 disabled:opacity-50 transition-all flex items-center justify-center gap-2">
...
export default function PCNListPage() {
  ...
  const CONTRACT_COLOR = { ARRS: "bg-blue-50 text-blue-700", EA: "bg-green-50 text-green-700", Direct: "bg-amber-50 text-amber-700", Mixed: "bg-purple-50 text-purple-700" };
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setModal("add")}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-purple-600 text-white text-[13px] font-semibold hover:bg-purple-700 transition-all">
            <Plus size={14}/> Add PCN
          </button>
        </div>
      </div>
          ...
          {pcns.map(pcn => (
            <div key={pcn._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-purple-200 transition-all p-5 flex items-center gap-4 group">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[15px] font-bold text-slate-800">{pcn.name}</p>
                <div className="flex items-center gap-3 mt-1 flex-wrap">
                  ...
                  {pcn.annualSpend > 0 && (
                    <span className="text-[11px] text-green-600 font-semibold flex items-center gap-1">
                      <DollarSign size={10}/> £{pcn.annualSpend.toLocaleString()}
                    </span>
                  )}
                  {pcn.xeroCode && (
                    <span className="text-[11px] text-slate-400">Xero: {pcn.xeroCode}</span>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                {pcn.contractType && (
                  <span className={`text-[10px] font-bold px-2 py-1 rounded-lg ${CONTRACT_COLOR[pcn.contractType] || "bg-slate-50 text-slate-600"}`}>
                    {pcn.contractType}
                  </span>
                )}
