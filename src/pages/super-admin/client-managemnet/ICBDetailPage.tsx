...
const EditRow = ({ label, value, onChange, type = "text" }: { label: string; value?: string | number | null; onChange: (val: string) => void; type?: string }) => (
...
export default function ICBDetailPage() {
  ...
  const OverviewPanel = () => {
    ...
    const CONTRACT_PILL: Record<string, string> = { ARRS:"bg-blue-100 text-blue-700", EA:"bg-green-100 text-green-700", Direct:"bg-amber-100 text-amber-700", Mixed:"bg-purple-100 text-purple-700" };
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(contractBreakdown).map(([type, count]) => (
                  <span key={type} className={`text-xs font-bold px-2.5 py-1 rounded-lg ${CONTRACT_PILL[type] || "bg-slate-100 text-slate-600"}`}>
                    {type}: {count as React.ReactNode} PCN{count !== 1 ? "s" : ""}
                  </span>
                ))}
              </div>
            </div>
  ...
  const PCNsPanel = () => {
    ...
    const totalSpend    = pcns.reduce((s: number, p: any) => s + (Number(p.annualSpend) || 0), 0);
    const totalPractices = pcns.reduce((s: number, p: any) => s + (p.practices?.length || 0), 0);
