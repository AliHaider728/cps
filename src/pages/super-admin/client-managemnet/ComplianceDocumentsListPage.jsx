...
import {
  FileText, Plus, Edit2, X, Check, Search, ExternalLink, Filter, Upload
} from "lucide-react";
import {
  useComplianceDocs,
...
const BadgeStatus = ({ ok, label }) => (
  <span className={`inline-flex items-center gap-1 rounded border px-3 py-1 text-sm font-semibold ${ok ? "border-green-200 bg-green-50 text-green-700" : "border-red-200 bg-red-50 text-red-600"}`}>
    {ok ? <Check size={14} /> : <X size={14} />}{label}
  </span>
);

const ActivePill = ({ active }) => (
  <span className={`rounded border px-3 py-1 text-sm font-semibold ${active ? "border-green-200 bg-green-50 text-green-700" : "border-red-200 bg-red-50 text-red-600"}`}>
...
const FilterBar = ({ search, setSearch, filters, setFilters, filterOptions }) => (
  <div className="flex flex-wrap items-center gap-3">
    ...
    {filterOptions.map((opt) => (
      <div key={opt.key} className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3">
        ...
        <select
          ...
          onChange={(e) => setFilters((current) => ({ ...current, [opt.key]: e.target.value }))}
    ...
    {(search || Object.values(filters).some(Boolean)) && (
      <button onClick={() => { setSearch(""); setFilters({}); }} className="flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold text-red-500 hover:bg-red-50 hover:text-red-700">
        <X size={15} /> Clear
      </button>
    )}
  </div>
      ...
      <div className="flex max-h-[92vh] w-full max-w-lg flex-col rounded-3xl border border-slate-200 bg-white shadow-2xl">
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-6 text-sm">
          ...
          <div>
            ...
            <input value={form.name} onChange={(e) => set("name")(e.target.value)} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-blue-400 focus:outline-none" />
          </div>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs font-bold uppercase tracking-wider text-slate-500">Display Order</label>
              <input type="number" value={form.displayOrder} onChange={(e) => set("displayOrder")(Number(e.target.value))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-blue-400 focus:outline-none" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-bold uppercase tracking-wider text-slate-500">Default Expiry Days</label>
              <input type="number" value={form.defaultExpiryDays} onChange={(e) => set("defaultExpiryDays")(Number(e.target.value))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-blue-400 focus:outline-none" />
            </div>
          </div>

          <div>
            <label className="mb-1 block text-xs font-bold uppercase tracking-wider text-slate-500">Default Reminder Days</label>
            <input type="number" value={form.defaultReminderDays} onChange={(e) => set("defaultReminderDays")(Number(e.target.value))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-blue-400 focus:outline-none" />
          </div>
          ...
          <div>
            ...
            <input type="file" onChange={(e) => setFile(e.target.files[0])} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm file:mr-4 file:rounded-xl file:border-0 file:bg-blue-100 file:px-5 file:py-2 file:text-sm file:text-blue-700" />
          </div>

          <div className="grid grid-cols-1 gap-4 pt-2 sm:grid-cols-3">
            <label className="flex cursor-pointer items-center gap-2 text-sm">
              <input type="checkbox" checked={!!form.mandatory} onChange={(e) => set("mandatory")(e.target.checked)} className="h-4 w-4 accent-blue-600" />
              <span>Mandatory</span>
            </label>
            <label className="flex cursor-pointer items-center gap-2 text-sm">
              <input type="checkbox" checked={!!form.expirable} onChange={(e) => set("expirable")(e.target.checked)} className="h-4 w-4 accent-blue-600" />
              <span>{form.expirable ? "Expirable" : "Non-Expirable"}</span>
            </label>
            <label className="flex cursor-pointer items-center gap-2 text-sm">
              <input type="checkbox" checked={!!form.active} onChange={(e) => set("active")(e.target.checked)} className="h-4 w-4 accent-blue-600" />
              <span>Active</span>
            </label>
          </div>
        </div>
        ...
        <div className="flex gap-3 border-t px-6 py-5">
          ...
          <button onClick={handle} disabled={saving || !form.name.trim()} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-blue-600 py-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50">
...
export default function ComplianceDocumentsListPage() {
  ...
  const filtered = docs
    .filter((doc) => {
      ...
      const matchSearch = !search || doc.name.toLowerCase().includes(q);
      const matchMandatory = !filters.mandatory || String(doc.mandatory) === filters.mandatory;
      const matchExpirable = !filters.expirable || String(doc.expirable) === filters.expirable;
      const matchActive = !filters.active || String(doc.active) === filters.active;
  ...
  const handleSaveDoc = async (formData) => {
    if (formData._id) {
      await updateDoc.mutateAsync({ id: formData._id, data: formData });
    } else {
  ...
  const columns = [
    {
      ...
      id: "name",
      render: (doc) => (
        <button onClick={() => navigate(`/dashboard/super-admin/compliance/documents/${doc._id}`)} className="flex items-center gap-1 font-medium text-blue-600 hover:text-blue-800 hover:underline">
          {doc.name}
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setDocModal({})} className="flex items-center gap-2 rounded-2xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition-all hover:bg-blue-700">
