  ...
  <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 py-2 border-b border-slate-50 last:border-0">
    ...
    {options ? (
      <select
        ...
        className="flex-1 px-2 py-1 rounded-lg border border-slate-200 text-[12px] text-slate-800 bg-white focus:outline-none focus:border-blue-400 transition-all cursor-pointer"
      >
        <option value="">—</option>
      ...
      <input
        ...
        value={value || ""}
        onChange={e => onChange(e.target.value)}
        className="flex-1 px-2 py-1 rounded-lg border border-slate-200 text-[12px] text-slate-800 bg-white focus:outline-none focus:border-blue-400 transition-all"
      />
    )}
  </div>
);
...
const ModalWrap = ({ title, onClose, children, footer }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-slate-200 max-h-[90vh] flex flex-col">
      ...
      {footer && (
        <div className="flex gap-3 px-5 pb-4 pt-3 border-t border-slate-100 shrink-0">
          {footer}
        </div>
      )}
    </div>
...
const ContactModal = ({ existing, onClose, onSave }) => {
    ...
    <div>
      ...
      {options ? (
        <select value={form[k]||""} onChange={e => setForm(f=>({...f,[k]:e.target.value}))}
          className="w-full px-3 py-2 rounded-xl border border-slate-200 text-[13px] bg-slate-50 focus:outline-none focus:border-blue-400 focus:bg-white transition-all cursor-pointer">
  ...
  return (
    <ModalWrap
      ...
      onClose={onClose}
      footer={<>
        <button onClick={onClose} className="flex-1 py-2 rounded-xl border border-slate-200 text-[13px] font-semibold text-slate-600 hover:bg-slate-50 transition-all">Cancel</button>
        <button onClick={handle} disabled={saving||!form.name.trim()}
          className="flex-1 py-2 rounded-xl bg-blue-600 text-white text-[13px] font-semibold hover:bg-blue-700 disabled:opacity-50 transition-all flex items-center justify-center gap-2">
    ...
    >
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        <input type="checkbox" checked={!!form.isPrimary} onChange={e=>setForm(f=>({...f,isPrimary:e.target.checked}))} className="accent-blue-600"/>
        <span className="text-[13px] text-slate-700 font-medium">Primary Contact</span>
      </label>
    </ModalWrap>
  );
};
...
const TemplateModal = ({ existing, onClose, onSave }) => {
  ...
  return (
    ...
    >
      {["name","subject"].map(k => (
        <div key={k}>
          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1 block">{k.charAt(0).toUpperCase()+k.slice(1)} *</label>
          <input value={form[k]||""} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))}
            className="w-full px-3 py-2 rounded-xl border border-slate-200 text-[13px] bg-slate-50 focus:outline-none focus:border-blue-400 focus:bg-white transition-all"/>
        </div>
      ))}
      <div>
        <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1 block">Body (HTML supported)</label>
        <textarea rows={8} value={form.body||""} onChange={e=>setForm(f=>({...f,body:e.target.value}))}
          className="w-full px-3 py-2 rounded-xl border border-slate-200 text-[13px] bg-slate-50 focus:outline-none focus:border-blue-400 focus:bg-white transition-all resize-none font-mono"/>
      </div>
    </ModalWrap>
  );
};
...
const MeetingModal = ({ existing, pcnId, onClose, onSave }) => {
  ...
  return (
    ...
    >
      ...
      {sel("type",[["monthly_review","Monthly Review"],["clinician_meeting","Clinician Meeting"],["governance","Governance"],["other","Other"]])}
      {sel("status",[["scheduled","Scheduled"],["completed","Completed"],["cancelled","Cancelled"],["not_booked","Not Booked"]])}
      <div>
        <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1 block">Attendees (comma separated)</label>
...
export default function PCNDetailPage() {
  ...
  // Modals
  const [contactModal,  setContactModal]  = useState(null); // null | {} | existing
  ...
  /* ── Patch helper (sends partial update to backend) ─────── */
  ...
  /* ── Contact CRUD (stored in PCN.contacts) ──────────────── */
  ...
  const OverviewTab = () => {
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Contract & Finance Card */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[12px] font-bold text-slate-500 uppercase tracking-wider">Contract & Finance</h3>
            <button
            ...
            <div className="space-y-0">
              {[
                ...
                <div key={l} className="flex justify-between gap-2 py-2 border-b border-slate-50 last:border-0">
                  ...
                  <span className="text-[12px] text-slate-800 font-semibold text-right max-w-[60%] truncate">{v}</span>
                </div>
              ))}
              {pcn.notes && (
                <div className="pt-3 mt-1">
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Notes</p>
                  <p className="text-[12px] text-slate-600 leading-relaxed">{pcn.notes}</p>
                </div>
              )}
            </div>
          )}
        ...
        {/* Required Systems — toggleable */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <h3 className="text-[12px] font-bold text-slate-500 uppercase tracking-wider mb-4">Required Systems</h3>
          ...
          {pcn.requiredSystems?.other && (
            <p className="text-[11px] text-slate-500 mt-3 pt-3 border-t border-slate-100">Other: {pcn.requiredSystems.other}</p>
          )}
          {saving && <p className="text-[11px] text-blue-500 mt-2 animate-pulse">Saving…</p>}
        </div>
      </div>
    );
  };
            ...
            <div key={c._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 group hover:border-blue-200 hover:shadow-md transition-all">
              <div className="flex items-start justify-between gap-2 mb-3">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {c.type && (
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-md border capitalize ${CONTACT_TYPE_STYLE[c.type]||CONTACT_TYPE_STYLE.general}`}>
                      {c.type.replace("_"," ")}
                    </span>
                  )}
                </div>
              ...
              <div className="space-y-1.5 mb-3">
                {c.email && (
                  <a href={`mailto:${c.email}`} className="flex items-center gap-2 text-[11px] text-blue-600 hover:text-blue-700 transition-colors truncate">
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity pt-2 border-t border-slate-100">
                <button onClick={() => setContactModal(c)}
                  className="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold text-blue-600 hover:bg-blue-50 transition-all">
                  <Edit2 size={10}/> Edit
                </button>
                <button onClick={() => deleteContact(c._id)}
                  className="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold text-red-500 hover:bg-red-50 transition-all">
                  <Trash2 size={10}/> Delete
                </button>
                {c.isPrimary && (
                  <span className="ml-auto text-[9px] bg-amber-50 text-amber-600 font-bold px-1.5 py-0.5 rounded-md border border-amber-200">Primary</span>
                )}
              </div>
            </div>
          ))}
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[13px] font-bold text-slate-700">PCN Compliance Score</h3>
          ...
          <p className="text-[11px] text-slate-400 mt-2">{done} of {checks.length} items completed · Click to toggle</p>
        </div>

        {saving && (
          <div className="flex items-center gap-2 text-blue-600 text-[12px] font-medium animate-pulse">
            <Spinner/> Saving…
          </div>
        )}
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-teal-600 text-white text-[11px] font-bold hover:bg-teal-700 transition-all">
          ...
          <div className="grid gap-2">
            {practices.map(p => (
              <button key={p._id}
                ...
                className="w-full bg-white rounded-2xl border border-slate-200 hover:border-teal-300 hover:shadow-sm transition-all px-4 py-3 flex items-center gap-4 group text-left">
              ...
              <div key={m._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex items-start gap-4 group">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-[13px] font-bold text-slate-800">{m.month}</p>
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md border ${STATUS_STYLE[m.status]||STATUS_STYLE.not_booked}`}>
                      {m.status?.replace("_"," ")}
                    </span>
                    <span className="text-[10px] text-slate-400">{TYPE_LABEL[m.type]||m.type}</span>
                  </div>
                  {m.date && <p className="text-[11px] text-slate-500 mt-1">{new Date(m.date).toLocaleDateString("en-GB",{weekday:"short",day:"2-digit",month:"short",year:"numeric"})}</p>}
                  {m.notes && <p className="text-[12px] text-slate-600 mt-1 leading-relaxed">{m.notes}</p>}
                  {m.attendees?.length > 0 && (
                    <p className="text-[11px] text-slate-400 mt-1.5">👥 {m.attendees.join(", ")}</p>
                  )}
                </div>
                <button onClick={() => setMeetingModal(m)}
                  className="opacity-0 group-hover:opacity-100 transition-opacity w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 hover:text-blue-600 hover:bg-blue-50">
                  <Edit2 size={12}/>
                </button>
              </div>
            ))}
              ...
              <div key={t._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 group">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5">Subject: {t.subject}</p>
                    {t.body && (
                      <p className="text-[11px] text-slate-400 mt-2 line-clamp-2 bg-slate-50 rounded-lg p-2">
                        {t.body.replace(/<[^>]+>/g,"")}
                      </p>
                    )}
                  </div>
    ...
    <div className="space-y-4 pb-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5">
        <div className="flex flex-wrap items-start gap-4">
          <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-purple-600 flex items-center justify-center shrink-0">
            <Network size={20} className="text-white"/>
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-lg sm:text-xl font-bold text-slate-800 leading-tight">{pcn.name}</h1>
            <div className="flex flex-wrap items-center gap-2 mt-1.5">
              {pcn.icb?.name && (
                <span className="text-[11px] text-slate-400 flex items-center gap-1">
                  <Building2 size={10}/> {pcn.icb.name}
                </span>
              )}
              {(pcn.federation?.name || pcn.federationName) && (
                <span className="text-[11px] text-slate-400 flex items-center gap-1">
                  <Layers size={10}/> {pcn.federation?.name || pcn.federationName}
                </span>
              )}
              {pcn.contractType && (
                <span className="text-[10px] bg-purple-50 text-purple-700 font-bold px-1.5 py-0.5 rounded-md border border-purple-200">
                  {pcn.contractType}
                </span>
              )}
              {pcn.annualSpend > 0 && (
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button onClick={() => navigate("/dashboard/super-admin/clients/pcn")}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 text-[11px] font-bold text-slate-600 hover:bg-slate-50 transition-all">
      ...
      {/* Tab bar — horizontally scrollable, no page scroll */}
