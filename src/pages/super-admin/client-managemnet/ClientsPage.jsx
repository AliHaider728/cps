...
const PracticeRow = ({ practice, navigate }) => (
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      <span className="text-sm font-semibold text-slate-700 group-hover:text-teal-700 truncate block">{practice.name}</span>
      {practice.odsCode && <span className="text-xs text-slate-400">ODS: {practice.odsCode}</span>}
    </div>
    <div className="flex items-center gap-2">
      {practice.contractType && (
        <span className="text-xs bg-slate-100 text-slate-500 font-bold px-2 py-0.5 rounded-md">{practice.contractType}</span>
      )}
...
const PCNCard = ({ pcn, navigate }) => {
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden mb-2">
      <button className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-slate-50 transition-colors text-left group" onClick={() => setOpen(o => !o)}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-0.5">
            {pcn.federation?.name && <span className="text-xs text-slate-400 flex items-center gap-1"><Layers size={10} />{pcn.federation.name}</span>}
            <span className="text-xs text-slate-400">{practiceCount} practice{practiceCount !== 1 ? "s" : ""}</span>
            {pcn.annualSpend > 0 && <span className="text-xs font-semibold text-green-600">£{pcn.annualSpend.toLocaleString()}</span>}
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {pcn.contractType && <span className="text-xs bg-purple-50 text-purple-600 font-bold px-2 py-0.5 rounded-md">{pcn.contractType}</span>}
          <button onClick={(e) => { e.stopPropagation(); navigate(`/dashboard/super-admin/clients/pcn/${pcn._id}`); }}
            className="text-xs font-bold text-blue-600 hover:text-blue-700 px-2.5 py-1 rounded-lg hover:bg-blue-50 transition-all">View</button>
          {practiceCount > 0 && <ChevronDown size={15} className={`text-slate-400 transition-transform duration-200 ${open ? "rotate-180" : ""}`} />}
        </div>
      </button>
      {open && practiceCount > 0 && (
        <div className="border-t border-slate-100 bg-slate-50/50 divide-y divide-slate-100">
          {pcn.practices.map(p => <PracticeRow key={p._id} practice={p} navigate={navigate} />)}
        </div>
      )}
    </div>
  );
...
const ICBSection = ({ icb, navigate }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-4">
      <button className="w-full flex items-center gap-4 px-6 py-5 hover:bg-slate-50 transition-colors text-left" onClick={() => setOpen(o => !o)}>
        <div className="w-11 h-11 rounded-xl bg-blue-600 flex items-center justify-center shrink-0"><Building2 size={20} className="text-white" /></div>
        <div className="flex-1 min-w-0">
          <p className="text-base font-bold text-slate-800">{icb.name}</p>
          <div className="flex items-center gap-4 mt-1 flex-wrap">
            {icb.region && <span className="text-xs text-slate-400 flex items-center gap-1"><MapPin size={11} />{icb.region}</span>}
        ...
        {icb.code && <span className="text-sm bg-blue-50 text-blue-700 font-bold px-2.5 py-1 rounded-lg shrink-0">{icb.code}</span>}
        <ChevronDown size={17} className={`text-slate-400 shrink-0 transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
      </button>
      {open && pcnCount > 0 && (
        <div className="px-6 pb-5 border-t border-slate-100 pt-4">
          {icb.pcns.map(pcn => <PCNCard key={pcn._id} pcn={pcn} navigate={navigate} />)}
        </div>
      )}
      {open && pcnCount === 0 && (
        <div className="px-6 pb-6 pt-3 border-t border-slate-100 text-center text-slate-400 text-sm">No active PCNs under this ICB</div>
      )}
    </div>
  );
};
  ...
  return (
    <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl border border-slate-200 shadow-xl z-50 overflow-hidden">
      {results.map((r, i) => {
        ...
        return (
          <button key={i} onClick={() => { navigate(getPath(r)); onClose(); }}
            ...
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${typeColor[r._type]}`}><Icon size={15} /></div>
            <div>
...
export default function ClientsPage() {
  ...
  const { data: searchData, isFetching: searching } = useSearchClients(debounced);
    ...
    <div>
      ...
      <div>
        ...
        {!tree.length ? (
          ...
          tree.map(icb => <ICBSection key={icb._id} icb={icb} navigate={navigate} />)
        )}
      </div>
    </div>
  );
