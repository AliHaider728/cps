...
const FilterChip = ({ label, children }: { label: string; children: React.ReactNode }) => (
...
const ExpiryField = ({ expirable, value, onChange, size = "md" }: { expirable?: boolean; value?: string; onChange: (val: string) => void; size?: "sm" | "md" }) => {
  const inputCls =
    size === "sm"
...
// Types for Document Row and Uploads
...
function UploadModal({ row, accent = "blue", onClose, onSave, saving }: {
  ...
  accent?: string;
  onClose: () => void;
  onSave: (args: { groupId: string; documentId: string; data: UploadPayload }) => Promise<any>;
  saving: boolean;
}) {
...
function UploadsModal({ row, accent = "blue", onClose, onSave, onDelete, onAdd, saving }: {
  ...
  accent?: string;
  onClose: () => void;
  onSave: (args: { groupId: string; documentId: string; uploadId: string; data: any }) => Promise<any>;
  onDelete: (args: { groupId: string; documentId: string; uploadId: string }) => Promise<any>;
  onAdd: (args: { groupId: string; documentId: string; data: UploadPayload }) => Promise<any>;
  saving: boolean;
}) {
  const [drafts, setDrafts] = useState<Record<string, { expiryDate: string; reference: string; notes: string; }>>(() =>
    Object.fromEntries(
  ...
  const saveUpload   = (uploadId: string) =>
  ...
  const handleReplaceClick = (uploadId: string) => {
...
function BulkUploadModal({ allDocuments, accent = "blue", onClose, onSave, saving }: {
  ...
  const [docStates, setDocStates] = useState<Record<string, { files: File[]; expiryDate: string; reference: string; notes: string; }>>({});
  ...
  const grouped = useMemo(() => {
    const map: Record<string, { groupId: string; groupName: string; docs: DocumentRow[] }> = {};
  ...
  const toggleDoc   = (docId: string) => setSelected((c) => ({ ...c, [docId]: !c[docId] }));
  ...
  const goToUpload = () => {
    ...
    setError("");
    const init: Record<string, { files: File[]; expiryDate: string; reference: string; notes: string; }> = {};
    selectedIds.forEach((id) => { init[id] = { files: [], expiryDate: "", reference: "", notes: "" }; });
    setDocStates(init);
    setStep(2);
  };

  const updateDoc = (docId: string, field: string, value: any) =>
...
export default function EntityDocumentsTab({ entityType, entityId, accent = "blue" }: {
  ...
  const [manageRow, setManageRow] = useState<DocumentRow | null>(null);
  ...
  const columns = [
    {
      ...
      render: (row: DocumentRow) => <span className="font-semibold text-slate-700">{row.groupName}</span>,
    ...
    },
    {
      ...
      render: (row: DocumentRow) => row.latestUpload ? (
        <div>
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-4">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-red-600">
              {(error as any)?.response?.data?.message || error.message || "Documents API request failed."}
            </p>
          </div>
        </div>
      )}
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="grid grid-cols-3 gap-2 text-center">
                {[
                  ...
                  <div key={label as string} className="min-w-[76px] rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                    <p className={`text-lg font-bold ${color}`}>{value as number}</p>
        ...
        <div className="flex flex-wrap gap-2">
          <FilterChip label="Group">
            <select value={filters.group}
              ...
              {groups.map((g: any) => <option key={g.groupId} value={g.groupId}>{g.groupName}</option>)}
            </select>
          </FilterChip>
      ...
      <DataTable
        columns={columns}
        data={rows}
        rowKey={(row: DocumentRow) => `${row.groupId}:${row.documentId}`}
        loading={false}
