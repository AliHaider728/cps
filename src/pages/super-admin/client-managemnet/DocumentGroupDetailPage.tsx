...
const Spinner: React.FC<SpinnerProps> = ({ cls = "border-white" }) => (
...
export default function DocumentGroupDetailPage() {
  ...
  const [form,    setForm]    = useState<DocumentGroupForm>({});
  ...
  useEffect(() => {
    if (group) {
      setForm({
        ...
        active:                  group.active,
        documents:               (group.documents || []).map((d: any) => d._id || d),
        // ── NEW FIELDS ─────────────────────────────────────────────────
  ...
  const set = (k: keyof DocumentGroupForm) => (v: any) => setForm(f => ({ ...f, [k]: v }));
  ...
  const activeDocs = allDocs.filter((d: any) => d.active);
  const allChecked = (form.documents || []).length === activeDocs.length;
  const toggleAll  = () => set("documents")(allChecked ? [] : activeDocs.map((d: any) => d._id));
  
  const cols: any[][] = [[], [], []];
  activeDocs.forEach((d: any, i: number) => cols[i % 3].push(d));
  ...
  const groupDocs = allDocs.filter((d: any) => (group.documents || []).some((gd: any) => (gd._id || gd) === d._id));

  const documentColumns = [
    { header: "Document Name", id: "name", render: (doc: any) => <span className="font-medium text-slate-800">{doc.name}</span> },
    { header: "Display Order", id: "displayOrder", render: (doc: any) => doc.displayOrder, cellClassName: "px-4 py-3 text-slate-500 align-top" },
    { header: "Mandatory",     id: "mandatory",    render: (doc: any) => <BadgeStatus ok={doc.mandatory} label={doc.mandatory ? "Mandatory" : "Non-Mandatory"} /> },
    { header: "Expirable",     id: "expirable",    render: (doc: any) => <BadgeStatus ok={doc.expirable} label={doc.expirable ? "Expirable" : "Non-Expirable"} /> },
    { header: "Active",        id: "active",       render: (doc: any) => <ActivePill active={doc.active} /> },
  ];
        ...
        <div className="flex flex-wrap items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-2 flex-wrap">
              ...
              {(group.applicableContractTypes || []).map((t: string) => (
            ...
            <div>
              ...
              {groupDocs.length === 0 ? (
                ...
                <DataTable
                  ...
                  data={[...groupDocs].sort((a: any, b: any) => a.displayOrder - b.displayOrder)}
                  rowKey="_id"
