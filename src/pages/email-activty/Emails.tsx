...
export default function Emails() {
  ...
  const filtered = useMemo(() => emails.filter((e: any) => {
    const q = search.toLowerCase();
    const matchSearch = !q || [e.subject, e.fromName, e.toName, e.fromEmail, e.clientName, e.bodyPreview].some((v: string) => v?.toLowerCase().includes(q));
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-3 flex-col sm:flex-row">
        <div className="relative flex-1">
          ...
          <Input value={search} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearch(e.target.value)} placeholder="Search emails, subjects, or clients..." className="pl-10" />
          {search && (
