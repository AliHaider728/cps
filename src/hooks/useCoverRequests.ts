...
export function useOpenCoverRequests(): UseQueryResult<unknown, Error> {
  return useQuery({
    queryKey: ["cover", "open"],
...
export function useCreateCoverRequest(): UseMutationResult<unknown, Error, Record<string, unknown>> {
...
export function useAssignCover(): UseMutationResult<unknown, Error, { id: string; clinician_id: string }> {
  const queryClient = useQueryClient();
  return useMutation({
